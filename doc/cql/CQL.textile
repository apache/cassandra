...
An @ALTER@ is used to manipulate with ColumnFamily columns. It allows you to add new column definitions and drop existing ones. No results are returned.

Remember that in Cassandra the column definition is always optional; the definition is only used for validation (and index creation). Columns that have not yet been defined can still be inserted, but Cassandra won't know how to validate them or provide type information to clients.
