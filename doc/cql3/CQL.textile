...
will request all rows that sorts after the one having "John's Blog" as @blog_title@ and '2012-01-01' for @posted_at@ in the clustering order. In particular, rows having a @posted_at <= '2012-01-01'@ will be returned as long as their @blog_title > 'John''s Blog'@, which wouldn't be the case for:
