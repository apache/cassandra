...
`enabled` to `false`
|`chunk_length_in_kb` |64 |On disk SSTables are
...
| `chunk_length` | 64KiB| alias to `chunk_length_in_kb` where size is specified with unit, available from CASSANDRA-12973.
|===
...
::= `=' | `<' | `>' | `<=' | `>=' | CONTAINS | CONTAINS KEY | NOT CONTAINS | NOT CONTAINS KEY +
::= (`,' )* +
::= ( `,' )* +
::= ( ASC | DESC )? +
::= `(' (`,' )* `)' +
