...
==== `WRITETIME`, `MINWRITETIME`, `MAXWRITETIME` and `TTL` function

Selection supports four special functions that aren't allowed anywhere
else: `WRITETIME`, `MINWRITETIME`, `MAXWRITETIME` and `TTL`.
