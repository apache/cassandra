...
[[writetime-function]]
==== `WRITETIME`, `MINWRITETIME` and `MAXWRITETIME` function

Selection supports three special functions that aren't allowed anywhere
else: `WRITETIME`, `MINWRITETIME` and `MAXWRITETIME`.
...
The `WRITETIME` function can be used on multi-cell columns such as non-frozen collections or non-frozen
user-defined types. In that case, the function will return the list of timestamps for each selected cell.
