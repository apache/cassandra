...
[NOTE]
====
the default source unit for `to_human_size` function is _bytes_, (`B`).
====

[source,cql]
----
cqlsh> select to_human_size(val) from ks.tb ;

 system.to_human_size(val)
---------------------------
                    58 KiB
                     1 MiB
                 11494 GiB
...
The second way to call `to_human_size` functions is to specify into what size unit we would like to see all
values to be converted to. For example, we want all size to be represented in mebibytes, hence we do:

[source,cql]
----
cqlsh> select to_human_size(val, 'MiB') from ks.tb ;

 system.to_human_size(val, 'MiB')
----------------------------------
                            0 MiB
                            1 MiB
                     11770573 MiB
...
[source,cql]
----
cqlsh> select to_human_size(val, 'Kib', 'MiB') from ks.tb ;

 system.to_human_size(val, 'Kib', 'MiB')
-----------------------------------------
                                  58 MiB
                                1205 MiB
                         12053067611 MiB
...
Similarly to `to_human_size`, we can do transformations on duration-like columns.

[NOTE]
====
the default source unit for `to_human_duration` function is _milliseconds_, (`ms`).
====

[source,cql]
----
cqlsh> select to_human_duration(val) from ks.tb ;

 system.to_human_duration(val)
-------------------------------
                           1 m
                          20 m
                      142851 d
...
[source,cql]
----
 system.to_human_duration(val, 'm')
------------------------------------
                                1 m
                               20 m
                        205705687 m
...
[source,cql]
----
cqlsh> select to_human_duration(val, 's', 'h') from ks.tb ;

 system.to_human_duration(val, 's', 'h')
-----------------------------------------
                                    16 h
                                   342 h
                            3428428120 h
