...
Transactions use strict null semantics, different from Paxos CAS:

[source,cql]
----
-- CAS behavior (Paxos): balance = null returns true when balance IS null
UPDATE users SET balance = 100 WHERE id = ? IF balance = null;
----

In transactions, `balance = null` is always FALSE. Use `IS NULL` instead:
...
BEGIN TRANSACTION
  ...
  -- Check for null, then check value
