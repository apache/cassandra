...
[TIP]
====
For query examples with `CONTAINS` clauses that take advantage of SAI collection maps, lists, and sets, be sure to see xref:developing/cql/indexing/sai/_collections.adoc#saiUsingCollections[SAI collection map examples with keys, values, and entries].
====
...
xref:developing/cql/indexing/sai/_sai-query.adoc[Using SAI] examines column index and query rules, plus collection maps.
