...
| `AUTHORIZE FOR` | `PERMISSION` | Authorize a user to `GRANT` and `REVOKE` permissions to another user
...
[[restrict-permissions-statement]]
=== RESTRICT STATEMENT

On separation of duties a user should be able to grant and revoke permissions but without having access to the data. The `RESTRICT` command allows the superuser to place such restriction. See `GRANT AUTHORIZE FOR` and Separation of duties for examples. 

[source, bnf]
----
include::example$BNF/restrict_statement.bnf[]
----

[[unrestrict-permissions-statement]]
=== UNRESTRICT STATEMENT

On separation of duties a user should be able to grant and revoke permissions but without having access to the data. The `RESTRICT` command allows the superuser to place such restriction. The `UNRESTRICT` command allows the superuser to modify such restriction.
