...
| `collection_count` | Any value | Gets the number of elements in a `map`, `set` or `list` argument. For other type of argument it returns `1`.

| `collection_min` | Any value | Gets the minimum element in a `set` or `list` argument. Any other type of not-collection argument is returned unmodified.

| `collection_max` | Any value | Gets the maximum element in a `set` or `list` argument. Any other type of not-collection argument is returned unmodified.

| `collection_sum` | Number or numeric `set` or `list` | Computes the sum of the elements in a `set` or `list` argument. Any other numeric not-collection argument is returned unmodified. The returned value is of the same type as the input collection elements, so there is a risk of overflowing the data type if the sum of the values exceeds the maximum value that the type can represent.

| `collection_avg` | Number or numeric `set` or `list` | Computes the average of the elements in a `set` or `list` argument. Any other numeric not-collection argument is returned unmodified. The average of an empty collection returns zero. The returned value is of the same type as the input collection elements, which might include rounding and truncations. For example `collection_avg([1, 2])` returns `1` instead of `1.5`.
|===
