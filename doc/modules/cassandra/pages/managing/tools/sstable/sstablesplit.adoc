...
Zero-copy splitting requires `storage_compatibility_mode: NONE` and a BIG
`pa`-or-later parent. It refuses uncompressed sstables, SSTable-attached indexes,
and components it cannot reproduce. Split children use the `pb` format version,
so do not use this option while a rolling upgrade is in progress.
