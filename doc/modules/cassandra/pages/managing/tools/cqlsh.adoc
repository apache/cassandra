...
`PAGING` accepts row counts only. Requesting xref:cassandra:developing/cql/dml.adoc#result-paging[byte paging] requires a client or driver that supports that feature.
