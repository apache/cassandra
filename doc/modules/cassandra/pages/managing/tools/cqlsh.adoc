...
`-i` `--interactive`::
  With -f or -e (or when reading from a non-interactive stdin) remain in an
  interactive prompt instead of exiting after the supplied commands complete.
  This option can also be enabled permanently in the `[ui]` section of cqlshrc
  by setting `interactive = true`. The command line flag always takes precedence
