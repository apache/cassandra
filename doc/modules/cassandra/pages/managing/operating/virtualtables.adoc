...
a virtual table `system_views.primary_ids` to allow users to query partition keys and related metadata for a specific table within a keyspace. This feature provides insights into SSTable-level details, such as token values, size estimates, and SSTable counts, without requiring expensive disk-based operations.
...
cassandra@cqlsh> select * from primary_ids where keyspace_name = 'ks' and table_name = 'tbl';
