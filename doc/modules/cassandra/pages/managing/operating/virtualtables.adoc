...
|caches |Displays the general weighted cache information including cache name, capacity_bytes, entry_count, hit_count, hit_ratio,
recent_hit_rate_per_second, recent_request_rate_per_second, request_count, and size_bytes.

|unweighted_caches | Displays the general unweighted cache information including cache name, capacity, entry_count, hit_count, hit_ratio, recent_hit_rate_per_second, recent_request_rate_per_second, and request_count
...
The `caches` virtual table lists information about weighted caches. The four
...
cqlsh> SELECT keyspace_name, table_name FROM tables;
----
+
results in:
+
...
-----------------------+----------------------------------
          ...
          system_views |                           caches
          system_views |    cidr_filtering_metrics_counts
          system_views | cidr_filtering_metrics_latencies
          ...
          system_views |           credentials_cache_keys
          ...
          system_views |             max_sstable_duration
          system_views |                 max_sstable_size
          system_views |   network_permissions_cache_keys
          system_views |                    pending_hints
          system_views |           permissions_cache_keys
          system_views |                          queries
          system_views |                      repair_jobs
          system_views |              repair_participates
          system_views |                  repair_sessions
          system_views |               repair_validations
          system_views |                          repairs
          system_views |                 roles_cache_keys
          ...
          system_views |                unweighted_caches
...
[source, cql]
----
batch_metrics                     pending_hints
caches                            permissions_cache_keys
cidr_filtering_metrics_counts     queries
cidr_filtering_metrics_latencies  repair_jobs
clients                           repair_participates
coordinator_read_latency          repair_sessions
coordinator_scan_latency          repair_validations
coordinator_write_latency         repairs
cql_metrics                       roles_cache_keys
credentials_cache_keys            rows_per_read
disk_usage                        sai_column_indexes
gossip_info                       sai_sstable_index_segments
internode_inbound                 sai_sstable_indexes
internode_outbound                settings
jmx_permissions_cache_keys        snapshots
local_read_latency                sstable_tasks
local_scan_latency                streaming
local_write_latency               system_logs
max_partition_size                system_properties
max_sstable_duration              thread_pools
max_sstable_size                  tombstones_per_read
network_permissions_cache_keys    unweighted_caches
----
...
------------------+---------------------------------------------------------------------------------------------------------------------
 address          | 127.0.0.1
 port             | 51218
 client_options   | {'COMPRESSION': 'lz4', 'CQL_VERSION': '3.4.7', 'DRIVER_NAME': 'DataStax Python Driver', 'DRIVER_VERSION': '3.25.0'}
 connection_stage | ready
 driver_name      | DataStax Python Driver
 driver_version   | 3.25.0
 hostname         | localhost
