...
|compaction_operations_status | Management compaction operations that are currently running or have been completed since
the node started. The default history size is 300, but can be changed by setting `cassandra.operations_history_size`
system property.

|compaction_operations_linked_tasks | Compaction operations that are currently running or have been completed since the
node started, enriched with compaction tasks statics if a compaction was involved.
