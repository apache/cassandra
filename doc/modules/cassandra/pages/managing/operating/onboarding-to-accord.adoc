...
provided otherwise the token ranges replicated on the node
you called the command on will be marked migrating.
If the entire range is marked migrating it is
...
ranges. If no range is specified it will default to all token ranges replicated by
the node for all keyspaces migrating to Accord.. This is the same behavior
as the default for repair.
