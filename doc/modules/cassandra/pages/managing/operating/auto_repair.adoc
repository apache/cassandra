...
major versions detected, which usually occurs when the cluster is being upgraded. Repair is not supported in this scenario
as data might be streamed from a node with a different version of Cassandra leading to compatibility issues. Once major
versions are compatible for streaming data, this setting can be turned to true.
|===
