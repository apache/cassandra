...
considered legacy and only supported for backward compatibility. If
both methods are used, a configuration error will occur at startup. Hot
reloading of the `SSLContext` is not yet supported for the JMX SSL.
...
`ssl_context_factory` in Cassandra's configuration.
