...
A few streaming counters are node wide rather than per peer, so they
carry no `<PeerIP>` scope:
...
zero-copy path as a synthesised slice covering only the requested token
ranges, rather than as a whole sstable.

|SlicedZeroCopyStreamsFailed |Counter |Number of times a slice could not
be synthesised, so the sstable fell back to partition-by-partition
streaming.
...
chunk plus anything between sections less than a chunk apart. This is
what `zero_copy_partial_stream_max_dead_space_ratio` trades against
deserializing rows.
|===
