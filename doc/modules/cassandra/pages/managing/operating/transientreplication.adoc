...
Being experimental, Witness replication is *not* recommended for
production use.
====
...
Witness replication is useful when sufficient full replicas are
unavailable to receive and store all the data. Witness replication
...
== Enabling Witness Replication

Witness replication is not enabled by default. Witness replication
...
Witness replication may be configured with both `SimpleStrategy` and
`NetworkTopologyStrategy`. Witness replication is configured by
...
Witnessly replicated keyspaces only support tables with `read_repair`
set to `NONE`.
...
Witness replicas can serve reads faster as they don't have to do
...
== Pending Ranges and Witness Replicas
...
full replicas. Witness replicas also always receive writes when they
are pending. Pending transient ranges are sent a bit more data and
reading from them is avoided.

== Read Repair and Witness Replicas
...
== Transitioning between Full Replicas and Witness Replicas
...
== Witness Replication supports EACH_QUORUM

(https://issues.apache.org/jira/browse/CASSANDRA-14727[CASSANDRA-14727])
adds support for Witness Replication support for `EACH_QUORUM`. Per
