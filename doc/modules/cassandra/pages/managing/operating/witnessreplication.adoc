...
initial implementation of witness replication. Future releases of
...
The objective of witness replication is to decouple storage
...
some nodes act as witness replicas, storing only unrepaired data for
the same token ranges.

The optimization that is made possible with witness replication is
...
writing data to a witness replica if we have successfully written data
to the full replicas.

After incremental repair, witness data stored on witness replicas
can be discarded.
...
....
witness_replication_enabled: true
....
...
setting replication factor as `<total_replicas>/<witness_replicas>`.
...
DC1, 1 of which is witness, and 5 replicas in DC2, 2 of which are
witness:
...
Witness replicated keyspaces only support tables with `read_repair`
set to `NONE`.
...
(no range movements).
* You can't add full replicas if there are any witness replicas. You
must first remove all witness replicas, then change the # of full
replicas, then add back the witness replicas.
* You can only safely increase number of witnesss one at a time with
incremental repair run in between each time.

Additionally, witness replication cannot be used for:
...
writing to witness replicas unless sufficient full replicas are not
available to satisfy the requested consistency level. Hints are never
written for witness replicas. Optimizations on the read path prefer
reading from witness replicas. When writing at quorum to a table
configured to use witness replication the quorum will always prefer
available full replicas over witness replicas so that witness
...
are slow or unavailable by sending writes to witness replicas.
...
for failure of one or more full replicas where witness replicas start
to serve a steady amount of write traffic for some of their witness ranges.

== Speculative Write Option

The `CREATE TABLE` adds an option `speculative_write_threshold` for use
with witness replicas. The option is of type `simple` with default
...
Pending ranges refers to the movement of token ranges between witness
replicas. When a witness range is moved, there will be a period of
time where both witness replicas would need to receive any write
intended for the logical witness replica so that after the movement
takes effect a read quorum is able to return a response. Nodes are _not_
temporarily witness replicas during expansion. They stream data like a
full replica for the witness range before they can serve reads. A
...
are pending. Pending witness ranges are sent a bit more data and
reading from them is avoided.

== Read Repair and Witness Replicas

Read repair never attempts to repair a witness replica. Reads will
always include at least one full replica. They should also prefer
witness replicas where possible. Range scans ensure the entire scanned
...
validation repair handling, at witness replicas anti-compaction does
not output any data for witness ranges as the data will be dropped
after repair, and witness replicas never have data streamed to them.

== Transitioning between Full Replicas and Witness Replicas

The additional state transitions that witness replication introduces
...
and not a witness replica.

Transitioning from not replicated to witness replicated means that a
...
Transitioning from witness replicated to fully replicated requires
...
managed so the witness replica is not read from as a full replica
until streaming completes. It can be used immediately for a write
quorum.

Transitioning from fully replicated to witness replicated requires
cleanup to remove repaired data from the witness replicated range to
reclaim space. It can be used immediately for a write quorum.

Transitioning from witness replicated to not replicated requires
cleanup to be run to remove the formerly witness replicated data.

When witness replication is in use ring changes are supported
...
consistency level. This is to minimally surprise users with witness
