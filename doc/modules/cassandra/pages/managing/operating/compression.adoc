...
* `--max-sampling-duration` (default: 600): Maximum time in seconds to
collect samples before training begins.
* `--sampling-rate` (default: 1.0): Sampling rate as a decimal between
0 and 1. A value of 1.0 samples all data, 0.5 samples 50%.
* `--async`: Run training asynchronously without waiting for completion.
* `--status`: Display current training status (NOT_STARTED, SAMPLING,
TRAINING, COMPLETED, FAILED).

NOTE: Consider running `nodetool flush` before training to ensure
recent writes are available for sampling from SSTable chunks.
