...
tables will be logged in the batch log to preserve all or nothing application.
However, atomicity of single token batch updates are still best effort.
Note when migrating to Accord, there is a performance hit for single token batch
updates that span both an Accord and non-Accord tables.
