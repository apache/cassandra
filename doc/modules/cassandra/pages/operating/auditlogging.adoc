...
If set to true, events generated by roles will be filtered by rates defiend in the
system_distributed.audit_users table.
...
Additionally, initialization parameters may be provided to the logger at construction via a map of strings. The logger implementation is expected to handle all edge cases assiciated with parsing key-value pairs from this map of string values.
...
OPTIONS
        --disable-role-filtering
            If set, role-generated-events will not be sampled
        ...
        --parameters <parameters>
            A stringified map representing a set of parameters to be passed to
            the logger's constructor. If not set the value from cassandra.yaml
            will be used
