...
* `role_filtering`: If true, this option enables role-based event filtering
...
`--parameters`::
  A stringified map representing a set of parameters to be passed to the
  logger's constructor. If not set the value from cassandra.yaml will be
  used
...
[source,none]
----
nodetool enableauditlog --logger <Default/ existing loggerName> --included-keyspaces <New Filter values>
----
