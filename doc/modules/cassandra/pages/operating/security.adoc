...
This is enabling certain users to manage permissions without having access to the data. Think of a hospital with patient data which is very sensitive information. IT should be able to grant/revoke/restrict access to the data without having access to the data itself. This is the clear separation of duties between admins and users of the database.
...
Notice the usages of `GRANT AUTHORIZE FOR`, `RESTRICT` and `UNRESTRICT` commands.
