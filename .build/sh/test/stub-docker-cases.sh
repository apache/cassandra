...
reset_stub
run_case "${cassandra_dir}/.build/docker/_docker_run.sh" bullseye-build.docker build-jars.sh --clean
...
reset_stub
run_case "${cassandra_dir}/.build/docker/_docker_run.sh" bullseye-build.docker build-jars.sh 8 --clean
...
expect_invocation "_docker_run.sh sets the given java version" "_set_java.sh 8"
expect_invocation "_docker_run.sh still passes --clean" "build-jars.sh --clean"
expect_no_invocation "_docker_run.sh does not pass the version to the run script" "build-jars.sh 8"

# a leading - marks a run script argument, so -s must never be read as a java version
reset_stub
run_case "${cassandra_dir}/.build/docker/_docker_run.sh" bullseye-build.docker build-jars.sh -s
...
expect_invocation "_docker_run.sh passes -s to build-jars.sh" "build-jars.sh -s"

reset_stub
run_case "${cassandra_dir}/.build/docker/_docker_run.sh" bullseye-build.docker build-jars.sh 99
...
reset_stub
run_case "${cassandra_dir}/.build/docker/_docker_run.sh" almalinux-build.docker docker/_build-redhat.sh 8
expect_rc "_docker_run.sh runs a run_script under a directory" 0
expect_invocation "_docker_run.sh names the container after the run_script alone" "\-\-name cassandra_almalinux-build_build-redhat_jdk8__"
expect_no_invocation "_docker_run.sh puts no directory separator in the container name" "\-\-name [^ ]*/"

# .build/docker/build-redhat.sh passes a quoted java version that can be empty, followed by
# the rpm_dist positional. an empty version must default, and must not shift rpm_dist away
reset_stub
run_case "${cassandra_dir}/.build/docker/_docker_run.sh" almalinux-build.docker docker/_build-redhat.sh "" rpm
expect_rc "_docker_run.sh runs with an empty java version and a trailing positional" 0
expect_out "_docker_run.sh defaults on an empty java version" "Defaulting to java ${java_default}"
# the empty argument stays in $@, so the command holds two spaces. the shell inside the
# container collapses them, and the run script still reads rpm as its first argument
expect_invocation "_docker_run.sh passes the trailing positional on" "_build-redhat.sh  *rpm"

# the same call site with the version given
reset_stub
run_case "${cassandra_dir}/.build/docker/_docker_run.sh" almalinux-build.docker docker/_build-redhat.sh 8 rpm
expect_rc "_docker_run.sh runs with a java version and a trailing positional" 0
expect_invocation "_docker_run.sh sets the java version before the positional" "_set_java.sh 8"
expect_invocation "_docker_run.sh passes only the positional to the run script" "_build-redhat.sh rpm"
expect_no_invocation "_docker_run.sh does not pass the version with the positional" "_build-redhat.sh 8"
...
reset_stub
run_case "${cassandra_dir}/.build/docker/run-tests.sh" -a test -t StorageServiceServerTest -j 8 -s
...
expect_invocation "docker/run-tests.sh sets java from -j" "_set_java.sh 8"

# the long flags must reach the container as the same short flags
reset_stub
run_case "${cassandra_dir}/.build/docker/run-tests.sh" --target test --test StorageServiceServerTest --java 8 --summary
...
expect_invocation "docker/run-tests.sh sets java from --java" "_set_java.sh 8"
...
reset_stub
run_case "${cassandra_dir}/.build/docker/run-tests.sh" test StorageServiceServerTest 8
expect_rc "docker/run-tests.sh runs three legacy arguments" 0
expect_out "docker/run-tests.sh shows the flags the legacy arguments became" "format: -a test -t StorageServiceServerTest -j 8"
expect_invocation "docker/run-tests.sh sets java from the third legacy argument" "_set_java.sh 8"
...
reset_stub
run_case "${cassandra_dir}/.build/docker/run-tests.sh" test 8
expect_rc "docker/run-tests.sh runs two legacy arguments" 0
expect_out "docker/run-tests.sh shows the second legacy argument as -j" "format: -a test  -j 8"
expect_invocation "docker/run-tests.sh sets java from the second legacy argument" "_set_java.sh 8"
expect_no_invocation "docker/run-tests.sh reads no test name regexp from it" "\-t 8"
