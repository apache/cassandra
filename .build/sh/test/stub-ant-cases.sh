...
reset_stub
for s in build-jars run-tests; do
...
run_case "${CASSANDRA_DIR}/.build/run-tests.sh" test 8
...
expect_out "run-tests.sh names the docker script that can" "docker/run-tests.sh -a test -j 8"
...
# 4.0's build-bench.xml has no microbench.exclude.pattern property and a private
# -microbench target, so _list_microbench_tests finds no tests and the microbench cases
# of the trunk harness cannot run here. restore them if 4.0's build-bench.xml is aligned.
...
# build-jars.sh calls run_ant; the entry names the script and the one ant target it runs
for _case in "build-jars.sh jar"; do
  s="${_case% *}"
  target="${_case#* }"
...
# _docker_run.sh puts -Dbuild.dir=${DIST_DIR} in ANT_OPTS, which build-jars.sh must not drop
reset_stub
run_case env ANT_OPTS="-Dbuild.dir=/sentinel" "${CASSANDRA_DIR}/.build/build-jars.sh" -s
expect_rc "build-jars.sh -s exits 0 with an inherited ANT_OPTS" 0
if grep -q -- "-Dbuild.dir=/sentinel" "${STUB_DIR}/ant-opts" 2>/dev/null; then
  pass "build-jars.sh keeps the ANT_OPTS it inherited"
else
  fail "build-jars.sh keeps the ANT_OPTS it inherited"
fi
