...
# 4.1's build-bench.xml has no microbench.exclude.pattern property and a private
# -microbench target, so _list_microbench_tests finds no tests and the microbench cases
# of the trunk harness cannot run here. restore them if 4.1's build-bench.xml is aligned.
