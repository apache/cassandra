...
#
#
# A wrapper script to run-tests-enhanced.sh (or dtest-python.sh) in docker.
...
[ -f "${cassandra_dir}/.build/run-tests-enhanced.sh" ] || { echo >&2 "${cassandra_dir}/.build/run-tests-enhanced.sh must exist"; exit 1; }
...
test_script="run-tests-enhanced.sh"
...
# HACK hardlinks in overlay are buggy, the following mount prevents hardlinks from being used. ref $TMP_DIR in .build/run-tests-enhanced.sh
...
case ${target} in
    ...
    "stress-test" | "fqltool-test" | "microbench" | "test-burn" | "test-burn-repeat" | "long-test" | "long-test-repeat" | "cqlsh-test" )
    ...
    ;;
    "test" |  "test-repeat" | "test-cdc" | "test-cdc-repeat" | "test-compression" | "test-compression-repeat" | "test-oa" | "test-oa-repeat" | "test-system-keyspace-directory" | "test-system-keyspace-directory-repeat" \
    | "test-tries" | "test-tries-repeat" | "jvm-dtest" | "jvm-dtest-repeat" |"jvm-dtest-upgrade" | "jvm-dtest-upgrade-repeat" )
        [[ ${mem} -gt $((5 * 1024 * 1024 * 1024 * ${jenkins_executors})) ]] || { echo >&2 "tests require minimum docker memory 6g (per jenkins executor (${jenkins_executors})), found ${mem}"; exit 1; }
...
docker_command="source \${CASSANDRA_DIR}/.build/docker/_set_java.sh ${java_version} ; \
            \${CASSANDRA_DIR}/.build/docker/_docker_init_tests.sh $* ; exit \$?"
