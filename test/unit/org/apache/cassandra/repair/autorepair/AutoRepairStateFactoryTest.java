...
public class AutoRepairStateFactoryTest
{
    ...
    public void testGetRepairState()
    {
        ...
        assertNotNull(state);
        assertTrue(state.getClass().getSimpleName().equals("FullRepairState"));

        state = RepairType.getAutoRepairState(RepairType.INCREMENTAL, new AutoRepairConfig());

        assertNotNull(state);
        assertTrue(state.getClass().getSimpleName().equals("IncrementalRepairState"));

        state = RepairType.getAutoRepairState(RepairType.PREVIEW_REPAIRED, new AutoRepairConfig());

        assertNotNull(state);
        assertTrue(state.getClass().getSimpleName().equals("PreviewRepairedState"));
    }
