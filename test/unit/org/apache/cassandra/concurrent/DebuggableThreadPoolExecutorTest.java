...
public class DebuggableThreadPoolExecutorTest
{
    ...
    public static void checkTracingIsPropagated(ExecutorPlus executor, Runnable schedulingTask) {
        ...
        Tracing.instance.set(new TraceState(FBUtilities.getLocalAddressAndPort(), nextTimeUUID(), Tracing.TraceType.NONE, false, true)
        {
            @Override
    ...
    private static void withTracing(Runnable fn)
    {
        ...
        try {
            Tracing.instance.set(new TraceStateImpl(InetAddressAndPort.getByAddress(InetAddresses.forString("127.0.0.1")), nextTimeUUID(), Tracing.TraceType.NONE, false, true));
            fn.run();
        }
        finally
        {
