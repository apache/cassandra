...
public class RangeFetchMapCalculatorTest
{
    ...
    {
        Multimap<Range<Token>, Replica> rangesWithSources = HashMultimap.create();
    ...
    private void validateRange(Multimap<Range<Token>, Replica> rangesWithSources, Multimap<InetAddressAndPort, Range<Token>> result)
    {
    ...
    private void addNonTrivialRangeAndSources(Multimap<Range<Token>, Replica> rangesWithSources, int left, int right, String... hosts) throws UnknownHostException
    {
        for (InetAddressAndPort endpoint : makeAddrs(hosts))
        {
            rangesWithSources.put(generateNonTrivialRange(left, right), Replica.full(endpoint));
        }
    }

    private void addTrivialRangeAndSources(Multimap<Range<Token>, Replica> rangesWithSources, int left, int right, String... hosts) throws UnknownHostException
    {
        for (InetAddressAndPort endpoint : makeAddrs(hosts))
        {
            rangesWithSources.put(generateTrivialRange(left, right), Replica.full(endpoint));
        }
    }
