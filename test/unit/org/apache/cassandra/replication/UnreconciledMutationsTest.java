...
public class UnreconciledMutationsTest
{
    ...
    private static Mutation createMutation(int partitionKey, int value, int offset)
    {
        ...
        long logId = 1L;
        long sequenceId = ((long) offset << 32) | 1000; // offset in high bits, timestamp in low bits
    ...
    public void testSingleTokenCollectionIsolation()
    {
        ...
        boolean found1 = unreconciled.collect(token1, tableId, false, offsets1);
        
        Assert.assertTrue("Should find mutations for token1", found1);
        Assert.assertEquals("Should only contain 1 offset for token1", 1, offsets1.offsetCount());
        ...
        boolean found2 = unreconciled.collect(token2, tableId, false, offsets2);
        
        Assert.assertTrue("Should find mutations for token2", found2);
        Assert.assertEquals("Should only contain 1 offset for token2", 1, offsets2.offsetCount());
        ...
        boolean found3 = unreconciled.collect(token3, tableId, false, offsets3);
        
        Assert.assertTrue("Should find mutations for token3", found3);
        Assert.assertEquals("Should only contain 1 offset for token3", 1, offsets3.offsetCount());
    ...
    public void testPendingVsVisibleMutations()
    {
        ...
        boolean foundVisible = unreconciled.collect(token, tableId, false, visibleOnly);
        
        Assert.assertTrue("Should find visible mutations", foundVisible);
        Assert.assertEquals("Should only have 1 visible mutation", 1, visibleOnly.offsetCount());
        ...
        boolean foundAll = unreconciled.collect(token, tableId, true, includingPending);
        
        Assert.assertTrue("Should find all mutations", foundAll);
        Assert.assertEquals("Should have 2 mutations total", 2, includingPending.offsetCount());
    ...
    public void testMultipleMutationsSameToken()
    {
        ...
        Assert.assertEquals("Should have all 3 mutations for same token", 3, offsets.offsetCount());
    ...
    public void testTableIdFiltering()
    {
        ...
        boolean foundCorrect = unreconciled.collect(token, tableId, false, correctTable);
        
        Assert.assertTrue("Should find mutation for correct table", foundCorrect);
        Assert.assertEquals("Should have 1 mutation", 1, correctTable.offsetCount());
    ...
    public void testMutationRemoval()
    {
        ...
        boolean foundBefore = unreconciled.collect(token, tableId, false, beforeRemoval);
        Assert.assertTrue("Should find mutation before removal", foundBefore);
        Assert.assertEquals("Should have 1 mutation before removal", 1, beforeRemoval.offsetCount());
        ...
        boolean foundAfter = unreconciled.collect(token, tableId, false, afterRemoval);
    ...
    public void testTokenRangeCollection()
    {
        ...
        boolean foundRange = unreconciled.collect(bounds, tableId, false, rangeOffsets);
    ...
    public void testSingleTokenRangeCollectionBug()
    {
        ...
        Assert.assertEquals("Single token range should only return 1 mutation", 1, offsets.offsetCount());
    ...
    public void testFullRangeCollectionWithMinimumToken()
    {
        ...
        Offsets.Mutable offsets = new Offsets.Mutable(new CoordinatorLogId(1, 1));
        boolean found = unreconciled.collect(fullRange, tableId, false, offsets);
        
        // Full range should return ALL mutations
        Assert.assertTrue("Should find mutations for full range", found);
        Assert.assertEquals("Full range should return ALL mutations", 3, offsets.offsetCount());
        Assert.assertTrue("Should contain mutation1 offset", offsets.contains(1));
    ...
    public void testSingleTokenCollectionVsFullRange()
    {
        ...
        boolean foundSingle = unreconciled.collect(targetToken, tableId, false, singleToken);
        
        Assert.assertTrue("Should find mutation for single token", foundSingle);
        Assert.assertEquals("Single token should only return 1 mutation", 1, singleToken.offsetCount());
