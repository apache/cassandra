...
public class RateLimitingTest extends NativeProtocolLimitsTestBase
{
    ...
    {
        // There can be non-0 unpaused connections from the previous test. We want to start from a clean slate and compare paused connections generated by this specific test
        int initialPausedConnections = getPausedConnectionsGauge().getValue();
        // The first query takes the one available permit.
        Message.Response firstResponse = client.execute(queryMessage(payloadSize));
        assertEquals(initialPausedConnections, getPausedConnectionsGauge().getValue().intValue());
