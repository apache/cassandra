...
public class ReplicaPlansTest
{
    ...
    public void testWriteEachQuorum()
    {
        final Token token = tk(1L);
        {
            ...
            ReplicaPlan.ForWrite plan = ReplicaPlans.forWrite(ks, ConsistencyLevel.EACH_QUORUM, (cm) -> natural, (cm) -> pending, null, Predicates.alwaysTrue(), ReplicaPlans.writeAll);
        ...
        }
        {
            ...
            EndpointsForToken pending = EndpointsForToken.empty(token);
            ReplicaPlan.ForWrite plan = ReplicaPlans.forWrite(ks, ConsistencyLevel.EACH_QUORUM, (cm) -> natural, (cm) -> pending, Epoch.FIRST, Predicates.alwaysTrue(), ReplicaPlans.writeAll);
            assertEquals(natural, plan.liveAndDown);
            assertEquals(natural, plan.live);
            assertEquals(natural, plan.contacts());
        }
