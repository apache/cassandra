...
public class ThrottlingOptionsTest
{
    ...
    public void testThrottlingOptions()
    {
        ...
        Assert.assertEquals(0.05, throttlingOptions.getPercentageOfTrafficToThrottling(), 0.0);
        Assert.assertEquals(2 * 60, throttlingOptions.getMoreAggressiveThrottlingAfterInSec());
        Assert.assertEquals(15 * 60, throttlingOptions.getResetAfterNoThrottlingSeenInSec());
        Assert.assertEquals(5, throttlingOptions.getAggressiveThrottlingQpsRatio(), 0.0);
        Assert.assertEquals(5, throttlingOptions.getAggressiveThrottlingLatencyRatio(), 0.0);
        Assert.assertEquals("^system.*\\..+|^pingless\\..+", throttlingOptions.getIgnoreTablesRegex());
        Assert.assertEquals(1, throttlingOptions.getHealthCheckPeriodInSec());
        Assert.assertEquals(60, throttlingOptions.getHealthCheckInitDelayInSec());
        Assert.assertTrue(throttlingOptions.getThrottleReadReplicaTraffic());
        Assert.assertFalse(throttlingOptions.getThrottleMutationReplicaTraffic());
