 ...
 * and the only thing the test decides is what that path finds; see {@link Outcome} for the choices (find nothing, find
 * arbitrary summaries, or fail).
 *
 * <p>{@link #discover} replaces {@link Loader#findKeysBetween} - whose default reads {@code commands_for_key} and so
 * needs a schema - with a set the test names; leave it unset for a key-domain task. {@link #relevance} answers
 * {@code loader.isRelevant(..)} directly, so that a key which is in range but not adopted can be stated rather than
 * inferred from an empty {@link accord.local.cfk.CommandsForKey}.
 *
 * <p>Install with {@link AccordCommandStore#unsafeOverrideRangeIndexFactory}. Outcomes are a function of the primary
 * txnId, so a task behaves the same way on every attempt.
 */
