 ...
 *
 * <p>The relation built here is {@code W = Wpos u Wlock} of {@code formalise/accord/execution/INVARIANTS.md} 2, and
 * nothing wider: a waiter must itself hold a position, the entry must not be loading (A1), and only a
 * {@code HOLD_QUEUE} holder yields a lock edge (A2). Anything looser invents edges, and so misdiagnoses a real hang.
 *
 * <p>Ahead is defined by {@link AccordCacheEntryQueue}'s regions, which is exactly
 * {@link AccordCacheEntry#unsafeQueuedTasks()} plus {@link AccordCacheEntry#unsafeRunnablePrefix()} - read here rather
 * than the queue array, as the two-claim case (i.e. the shape a stall has) is a mini queue rather than an
 * {@code AccordCacheEntryQueue}. Sorted order is a function of the pair compared, so two tasks sharing two entries
 * agree on both, and a cycle means some region is ordering by something other than {@code compare}.
 *
 * <p>Two failure modes produce the same symptom, so both are reported: a genuine cycle, and a task whose
 * {@code waitingForState} does not match the number of entries it is blocked on.
 */
