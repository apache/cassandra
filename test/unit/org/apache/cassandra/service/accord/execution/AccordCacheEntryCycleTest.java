 ...
 * across runs with {@code HOLD_QUEUE}) and a position edge (the task is not in the entry's runnable prefix) - and a
 * deadlock is a cycle in their union.
 *
 * <p>The smallest deadlock is two tasks and two entries: {@code H} holds a txnId across its runs and {@code W} queues
 * behind it there, while on a shared key {@code W} sits ahead of {@code H}. This is prevented by upgrade-on-start:
 * {@code H} becomes a fifo claim on every entry it holds when it starts running, and fifo precedes sorted precedes
 * bag, so {@code W} can only be ahead of it if it is itself a fifo claim - which an ATOMIC {@code W} cannot be on one
 * entry and not the other, as it claims everything it declared in one pass (hence {@link #isLegalOrder}).
 *
 * <p>These are ordering bugs, so the interleavings are enumerated rather than chosen: for each region {@code H} and
