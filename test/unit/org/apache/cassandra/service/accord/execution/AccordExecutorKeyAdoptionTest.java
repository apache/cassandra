 ...
 *
 * The range-domain half of a task's lifecycle, and in particular key adoption: a task that declares {@code Ranges}
 * runs a {@code SafeTask.RangeTxnAndKeyScanner}, which registers a {@code KeyWatcher} on the commands-for-key cache
 * for the duration of the scan and may adopt a reference to a key inside its ranges mid-flight.
 *
 * <p>Adoption is the one place a task's reference set grows after setup, and so the one place the at-once acquisition
 * premise of the cycle-freedom argument can be broken: a task must hold exactly one queue position per referenced
 * entry. Both directions are pinned by {@link #assertQueuesConsistent} - adopting without a position lets the task run
 * with a key it does not lead; adopting with two makes it wait for itself and leak a position.
 *
 * <p>The watcher is live from {@code startInternal} until {@code finish}, so adoption is reachable in each state named
 * by {@link Arrival}. Which keys the scan discovers ({@link ControllableRangeIndex#discover}), when it finishes, which
...
public class AccordExecutorKeyAdoptionTest
{
    ...
    {
        AccordCommandStore.unsafeOverrideRangeIndexFactory = null;
    }
     ...
     *
     * <ul>
     *   <li>{@code LOADING_REQUIRED} is sync-only (a batched task's keys are optional) and {@code LOADING_OPTIONAL}
     *       batched-only.</li>
     *   <li>{@link Source#MODIFIED_RELEASE} needs a task to run and release, and an {@code ExclusiveExecutor} serves one
     *       task per store at a time - which is the lever {@code WAITING_TO_RUN} uses to hold itself open.</li>
     *   <li>{@code WAITING_ON_TXN} is INCR-only, and {@code WAITING_ON_KEY} absent: waiting needs a claim in the fifo
     *       region or sorted prefix, and two top-level tasks on one store never block each other on a key, as the
     *       store's run slot already serialises them. That state belongs in a harness that sets it directly (see
     *       {@code AccordCacheEntrySafeTaskCycleTest}).</li>
     * </ul>
     */
    @Test
     ...
     *
     * <p>This is the one path that reaches the bit without declaring the key, as a range task discovers its keys
     * mid-flight and so cannot be refused at {@code setupExclusive}. The entry is marked while it is still loading with
     * the scan live, so {@code Listener::onUpdate} fires from {@code loaded()} and reaches the watcher; marking it any
     * earlier would refuse the loader instead, with no notification at all.
     */
    @Test
    ...
    private static class Fixture implements AutoCloseable
    {
        ...
        Fixture()
        {
            ...
            AccordCommandStore.unsafeOverrideRangeIndexFactory = cs -> {
