...
public class AccordExecutorLoadFailureTest
{
     ...
     * waits for a task that will never run again. Several statements in {@code SafeTask.completeExclusiveMayThrow} can
     * throw ahead of its final release of references, and {@code Task.completeExclusiveNoExcept} used only to report
     * the throwable and fail the task - so it stayed FAILED for ever holding a position on every entry in
     * {@code refs}, while the executor's accounting said it was gone.
     *
     * <p>Provoked via {@code nonSync().postRunExclusive(this)}: releasing the keys of the batch just run notifies the
     * commands-for-key cache listeners before removing the task's position, so a listener that throws lands where a
     * real one would (the range scanner's {@code KeyWatcher} is such a listener). The task is ASYNC over two loaded
     * keys with a batch size of one, so it throws with the second key and its Command entry still cleanly claimed; a
     * later task on that second key must then run within {@link #TIMEOUT_SECONDS}, and R6 is also asserted directly by
     * reading {@code AccordCacheEntry.contains(task)} under the executor lock.
     *
     * <p>Nothing is asserted about the key of the batch itself: the throw interrupts the middle of its release, which
     * no fix on the catch path can complete.
     */
    @Test
