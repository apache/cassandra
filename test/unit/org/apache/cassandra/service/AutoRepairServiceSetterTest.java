...
//        DatabaseDescriptor.setConfig(DatabaseDescriptor.loadConfig());
    ...
    public static void setup() throws Exception
    {
        ...
        // Disable materialized view repair to avoid conflicts with STREAMING_REQUIRES_VIEW_BUILD_DURING_REPAIR
        config.setMaterializedViewRepairEnabled(AutoRepairConfig.RepairType.INCREMENTAL, false);
    }
    ...
    public void testSettersTest()
    {
        ...
        if (actualConfig instanceof Set)
        {
            ...
            if (arg instanceof Set)
                assertThat(((Set<?>) actualConfig).stream().anyMatch(entry -> arg.toString().contains(entry.toString()))).isTrue();
            else
                assertThat(((Set<?>) actualConfig).stream().anyMatch(entry -> entry.toString().contains(arg.toString()))).isTrue();
        }
        else
