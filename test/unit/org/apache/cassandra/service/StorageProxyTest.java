...
public class StorageProxyTest
{
    ...
    private void setExecutors(ExecutorPlus mockHintExecutor, ExecutorPlus mockMutationExecutor) throws Exception {
        ...
        // Get the original HINT and MUTATION executors
        ExecutorPlus originalHintExecutor = (ExecutorPlus) executorField.get(Stage.HINT);
        ExecutorPlus originalMutationExecutor = (ExecutorPlus) executorField.get(Stage.MUTATION);
    ...
    @Test
    public void testHintIsPutInMutationQueueAndNotHintQueueOnConfigChange() throws Exception {
        // set config to use MUTATION stage for hints
        DatabaseDescriptor.setUseHintStageForHints(false);
        ...
        // Assertions
        Assert.assertEquals(1, hintsAfter - hintsBefore);
        verify(mockMutationExecutor, times(1)).submit(any(Runnable.class));
        verify(mockHintExecutor, never()).submit(any(Runnable.class));

        DatabaseDescriptor.setUseHintStageForHints(true);
    }
}
