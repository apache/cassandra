    ...
    private void innerTestMultiThreadedReadConsistency() throws InterruptedException
    {
        ...
        Thread t1 = new Thread(new Runnable()
        {
            public void run()
            {
                ...
                state.addApplicationState(ApplicationState.STATUS_WITH_PORTS, valueFactory.normal(tokens));
            }
        });
        ...
        Thread t2 = new Thread(new Runnable()
        {
            public void run()
            {
                for (int i = 0; i < 50; i++)
                {
                    ...
                    if (values.containsKey(ApplicationState.STATUS_WITH_PORTS) && !values.containsKey(ApplicationState.TOKENS))
                    {
    ...
    private void innerTestMultiThreadWriteConsistency() throws InterruptedException
    {
        ...
        Thread t1 = new Thread(new Runnable()
        {
            public void run()
            {
                ...
                states.put(ApplicationState.STATUS_WITH_PORTS, valueFactory.normal(tokens));
        ...
        assertTrue(values.containsKey(ApplicationState.STATUS_WITH_PORTS));
