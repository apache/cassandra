...
public class SchemaTestUtil
{
    ...
    private static void announceNewTable(TableMetadata cfm, boolean throwOnDuplicate)
    {
        ...
        Schema.instance.submit((metadata, isReplay) -> metadata.schema.getKeyspaces().withAddedOrUpdated(ksm.withSwapped(ksm.tables.with(cfm))));
    }
    ...
    static void announceKeyspaceUpdate(KeyspaceMetadata ksm)
    {
        ...
        Schema.instance.submit((metadata, isReplay) -> metadata.schema.getKeyspaces().withAddedOrUpdated(ksm));
    }
    ...
    public static void announceTableUpdate(TableMetadata updated)
    {
        ...
        Schema.instance.submit((metadata, isReplay) -> metadata.schema.getKeyspaces().withAddedOrUpdated(ksm.withSwapped(ksm.tables.withSwapped(updated))));
    }
    ...
    static void announceKeyspaceDrop(String ksName)
    {
        ...
        Schema.instance.submit((metadata, isReplay) -> metadata.schema.getKeyspaces().without(ksName));
    }

    public static SchemaTransformation dropTable(String ksName, String cfName)
    {
        return (metadata, isReplay) -> {
    ...
    {
        Schema.instance.submit((metadata, isReplay) -> metadata.schema.getKeyspaces().without(Collections.singletonList(ksName)));
    }
}
