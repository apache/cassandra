...
public class TOCComponentTest
{
    ...
    public void testAppendComponentsInSortedOrder() throws Exception
    {
        ...
        "CompressionInfo.db",
        "CRC.db", // require case-insensitive sorting to not be the first
        "Data.db",
        "Digest.crc32",
        "Filter.db",
        "Statistics.db",
    ...
    public void testRewriteTOCReplacesFileWithSortedComponents() throws Exception
    {
        ...
        java.io.File bf = tempDir.newFile("nb-1-big-Data.db");
        Descriptor descriptor = Descriptor.fromFile(new File(bf));

        File tocFile = descriptor.fileFor(Components.TOC);
        Files.write(tocFile.toPath(), Arrays.asList("Data-OLD.db", "Filter-OLD.db", "TOC-OLD.txt"));
