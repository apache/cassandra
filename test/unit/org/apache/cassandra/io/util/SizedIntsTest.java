    ...
    public void readWrite()
    {
        try (DataOutputBuffer out = new DataOutputBuffer(8))
        {
            qt().forAll(Generators.bytes(1, 8).map(bb -> new BigInteger(ByteBufferUtil.getArray(bb)).longValue()).mix(longs().all())).check(v -> {
                ...
                try
                {
                    SizedInts.write(out, v, v == 0 ? 0 : SizedInts.nonZeroSize(v));
                    out.flush();
                    long r = SizedInts.read(out.asNewBuffer(), 0, v == 0 ? 0 : SizedInts.nonZeroSize(v));
                    return v == r;
                }
                catch (IOException e)
                {
    ...
    public void readWriteUnsigned()
    {
        try (DataOutputBuffer out = new DataOutputBuffer(8))
        {
            ...
            qt().forAll(Generators.bytes(1, 8).map(bb -> new BigInteger(ByteBufferUtil.getArray(bb)).longValue()).mix(longs().all())).check(v -> {
                ...
                try
                {
                    int size = v == 0 ? 0 : SizedInts.nonZeroSize(v);
