 ...
 * The splitter is BIG-only (BTI has no rebaseable {@code Index.db}), so every case that expects -- or merely
 * asserts the eligibility of -- the zero-copy path is gated on {@link BigFormat#isSelected()}. The case that is
 * genuinely format agnostic ({@link #uncompressedSSTableFallsBackToTheRewritePath}) deliberately runs under
 * whichever format is selected, and {@link #btiSSTableFallsBackToTheRewritePath} forces BTI on purpose.
 * {@code zero_copy_anticompaction_enabled} defaults to <b>false</b>, so every test sets it explicitly (in both
 * directions) and the value is saved and restored around each one.
 */
public class ZeroCopyAntiCompactionTest extends CQLTester
{
    ...
    /**
     * Trunk only, and the one regression risk the fork could not have had: BTI is a real, selectable format and
     * <em>nothing</em> in the splitter can work for it (the technique rebases Index.db partition positions, and
     * BTI has no Index.db). The gate must therefore refuse it -- with a reason naming the format, and without
     * throwing -- and the unchanged rewrite path must run and produce a completely correct result. The
     * {@link #assertOutcome} oracle, including the per-partition repair-state check, is applied unchanged.
     */
    @Test
    public void btiSSTableFallsBackToTheRewritePath() throws Throwable
    {
        ...
        try
        {
            ...
            assertFalse("a BTI sstable must never be splittable", ZeroCopySSTableSplitter.isSupported(parent));
            ...
            // the planner must refuse rather than throw, and must say why in terms an operator can act on
            AntiCompactionRunPlanner.Plan plan = AntiCompactionRunPlanner.plan(parent, ranges, sessionID);
            assertFalse("the planner declared a BTI sstable eligible: " + plan, plan.eligible);
            assertNotNull("an ineligible plan must say why", plan.ineligibleReason);
            assertTrue("the reason must name the offending format: " + plan.ineligibleReason,
                       plan.ineligibleReason.contains(BtiFormat.NAME));
            ...
            assertEquals("a BTI sstable must not take the zero-copy path", 0, zeroCopyBytes(cfs));
            // 2, the rewrite result: one pending and one unrepaired. The split would have produced 3.
            assertOutcome(cfs, parent, before, full, Collections.emptySet(), sessionID, 2);
        }
        finally
        {
