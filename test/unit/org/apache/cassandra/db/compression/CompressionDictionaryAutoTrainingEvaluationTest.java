...
import static org.apache.cassandra.io.sstable.metadata.MetadataCollector.NO_COMPRESSION_RATIO;
...
public class CompressionDictionaryAutoTrainingEvaluationTest
{
    ...
    /**
     * computeSampleCompressionRatio returns NO_COMPRESSION_RATIO (-1.0) when nothing was sampled, and -1.0 is
     * finite, so it reaches the improvement arithmetic. Unguarded it inverts the comparison rather than failing:
     * the relative improvement formula turns a -1.0 on either side into a large positive number.
     */
    @Test
    public void unmeasurableBaselineDoesNotPromote()
    {
        // (-1.0 - 0.30) / -1.0 = +1.30 -> would clear any threshold on a baseline that measured nothing
        assertThat(isPromoted(NO_COMPRESSION_RATIO, 0.30)).isFalse();
    }

    @Test
    public void unmeasurableCandidateDoesNotPromote()
    {
        // (0.50 - -1.0) / 0.50 = +3.00 -> the worst case: a candidate that measured NOTHING wins by the
        // largest possible margin
        assertThat(isPromoted(0.50, NO_COMPRESSION_RATIO)).isFalse();
    }

    /**
     * These two are safe even without the guard - (-1.0 + 1.0) / -1.0 is -0.0, and a zero baseline gives
     * -Infinity - so they are here to keep that accidental safety pinned down rather than relied upon.
     */
    @Test
    public void bothUnmeasurableDoesNotPromote()
    {
        assertThat(isPromoted(NO_COMPRESSION_RATIO, NO_COMPRESSION_RATIO)).isFalse();
    }

    @Test
    public void zeroBaselineDoesNotPromote()
    {
        assertThat(isPromoted(0.0, 0.30)).isFalse();
    }
