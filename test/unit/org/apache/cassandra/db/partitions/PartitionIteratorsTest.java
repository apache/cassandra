...
{
    private SinglePartitionReadCommand mockReadQuery()
    {
        SinglePartitionReadCommand readQuery = mock(SinglePartitionReadCommand.class);
        CFMetaData metadata = CFMetaData.Builder.create("ks", "table")
                                                .withPartitioner(Murmur3Partitioner.instance)
                                                .addPartitionKey("id", Int32Type.instance)
                                                .build();
    ...
    public void testPartitionIteratorIsClosed()
    {
        ...
        SinglePartitionReadCommand readQuery = mockReadQuery();

        Row r = mock(Row.class);
        when(r.isRow()).thenReturn(true);

        RowIterator ri = mock(RowIterator.class);
        Iterator<Row> i1 = Arrays.asList(r).iterator();
