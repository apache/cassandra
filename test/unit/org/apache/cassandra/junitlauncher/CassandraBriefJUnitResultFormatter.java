...
/**
 * Prints plain text output of the test to STDOUT. It is inspired by
 * {@code org.apache.tools.ant.taskdefs.optional.junitlauncher.LegacyBriefResultFormatter}. We could not extend the
 * existing class, see https://github.com/apache/ant/pull/169 for details. Here is the list of changes compared to the
 * original class: 1) test tag support 2) keep brief brief support 3) writing to STDOUT instead of a writer.
 */
public class CassandraBriefJUnitResultFormatter extends AbstractJUnitResultFormatter
{
    ...
    {
        // Here we intentionally substitute the original OuputStream by STDOUT because we do not want to write to output
        // file. In fact, an empty file is still being created, see https://github.com/apache/ant/pull/171 for details.
    ...
    private String determineTestSuiteName(Optional<ClassSource> testClass)
    {
        return testClass.map(ClassSource::getClassName)
                        .map(name -> !tag.isEmpty() ? name + '-' + tag : name)
                        .orElse("UNKNOWN");
    }

    protected boolean shouldReportExecutionFinished(@SuppressWarnings("unused") final TestIdentifier testIdentifier,
