...
public class ProgressBarrierTest extends CMSTestBase
{
    static
    {
        System.setProperty(CassandraRelevantProperties.PROGRESS_BARRIER_BACKOFF_MILLIS.getKey(), "5");
    }
    ...
    public void testProgressBarrier() throws Throwable
    {
        ...
        Supplier<ConsistencyLevel> cls = Surjections.pick(ConsistencyLevel.QUORUM,
                ...
                for (int check = 0; check < 10; check++)
                {
                    ...
                    switch (cl)
                    {
                        ...
                        case ONE:
                            ...
                            Assert.assertTrue(String.format("Should have colleced at least one of the replicas %s, but got %s",
                                                            replicas, responded),
    ...
    {
        System.setProperty(CassandraRelevantProperties.PROGRESS_BARRIER_TIMEOUT_MILLIS.getKey(), "5");
        PlacementSimulator.ReplicationFactor rf = new PlacementSimulator.NtsReplicationFactor(5, 5);
        PlacementSimulator.NodeFactory nodeFactory = PlacementSimulator.nodeFactory();

        DatabaseDescriptor.setProgressBarrierLowestAcceptableConsistencyLevel(ConsistencyLevel.ONE);
        try (CMSTestBase.CMSSut sut = new CMSTestBase.CMSSut(AtomicLongBackedProcessor::new, false, rf))
        {
            PlacementSimulator.Node node = null;
            for (int i = 1; i <= 4; i++)
