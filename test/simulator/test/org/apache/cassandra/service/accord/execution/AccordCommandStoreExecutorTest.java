 ...
 * functions that return an uninitialised value rather than reading from {@code system_accord}.
 *
 * <p>{@value #STORES} command stores over {@value #EXECUTORS} executors (so some stores share one), all sharing one
 * key space; each submission may nest one task on the same store, another store on the same executor, or a store on
 * another executor. Verifies: liveness; that a running task references every key and txnId it declared, and only for
 * its own store; mutual exclusion over any key or txnId of one store; that work submitted by a running task is
 * attached to it iff it belongs to the same executor; one executor per thread; that every task is assigned its
 * context's {@link ExecutionSequence} (a lower bound on the ordering imposed, not an exact one) and that siblings
 * sharing an entry run in the order the queues promise; ASYNC/INCR batching; load failure handling; and behaviour
 * under a cache far smaller than the working set.
 *
 * <p>Only the {@code SIGNAL} and {@code ASYNC} submission models can be simulated; {@code SYNC}/{@code SEMI_SYNC}
 * (the production default) need the simulator to intercept {@link java.util.concurrent.locks.ReentrantLock}. Each
...
public class AccordCommandStoreExecutorTest extends SimulationTestBase
{
    ...
    /**
     * How often a task declares {@code Ranges} rather than {@code RoutingKeys}: its keys are then discovered by
     * {@code RangeTxnAndKeyScanner} rather than declared, and its {@code KeyWatcher} may adopt a reference to a
     * relevant key while the scan runs - the only place a task's reference set grows after setup. Only every other
     * key is discoverable (see {@code Harness.isDiscoverable}), so the rest can only arrive by adoption.
     */
     ...
     * off, so no task queues on an entry and no key-level ordering is expected; what is validated is that the
     * off-switch is whole - every task still runs, completes, saves and releases. Batching is defined over the queue
     * positions, so this disables non-sync execution too (see {@link #effectiveLoadKeys}), and an atomic task takes no
     * fifo position and so provides no atomicity (see {@link Harness#verifySequenced}).
     */
    @Test
    ...
    /**
     * The same workload with {@code queue_nonsync_enabled = false} but the cache queues left on: every task runs as
     * {@code SYNC} and is ordered by the entry queues as any other sync work is; only batched-run coverage is dropped.
     */
    @Test
    ...
    static class Harness
    {
        ...
        {
            AccordCommandStore.unsafeOverrideRangeIndexFactory = store -> newRangeIndex(store);
        }
        ...
        /**
         * A task is assigned its context's {@link ExecutionSequence} whether or not it was pre-set-up: preSetup does it
         * for a child on its parent's own store, and submitExclusiveMayThrow for everything else.
         * <p>
         * Asserts the specified guarantee rather than the sequence bits, as a task may legally end up more strongly
         * ordered than it declared: the fifo upgrade at first prepare stops an upgraded BY_PRIORITY task reporting
         * {@code isSequencedByPriority()}, which is within spec.
         */
    ...
    /**
     * Verify the execution order of each batch's children. The contract guarantees nothing about it - atomicity is
     * with respect to the parent - so this is a canary for what the queues currently produce: an atomic child
     * submitted before a prioritised sibling runs first, atomic siblings run in submission order, and prioritised
     * siblings run in the queue's comparison order. An atomic child submitted after a prioritised sibling is counted
     * rather than required, as it would have to revoke a sibling that may already have locked its keys.
     */
