...
/**
 * Paxos simulation test with mutation tracking enabled (without witnesses).
 *
 * Note: Topology changes are disabled (--cluster-actions "") because
 * they are not currently supported with mutation tracking. We use
 * --cluster-action-limit -1 to ensure all nodes join the ring.
 *
 ...
 */
public class ShortPaxosTrackingSimulationTest
{
    @Test
    public void simulationTestV1()
    {
        PaxosSimulationRunner.executeWithExceptionThrowing(new String[] {
            ...
            "--cluster-actions", "",
            "--replication-config", "tracking",
            "-s", "30"
        });
    }

    @Test
    public void simulationTestV2()
    {
        PaxosSimulationRunner.executeWithExceptionThrowing(new String[] {
        "run",
        "--variant", "v2",
    ...
    @Test
    public void simulationTestV1NoForwarding()
    {
        PaxosSimulationRunner.executeWithExceptionThrowing(new String[] {
            "run",
            "--variant", "v1",
            ...
            "--cluster-actions", "",
            "--replication-config", "tracking",
            "--disable-consensus-forwarding",
            "-s", "30"
        });
    }

    @Test
    public void simulationTestV2NoForwarding()
    {
        PaxosSimulationRunner.executeWithExceptionThrowing(new String[] {
            "run",
            "--variant", "v2",
