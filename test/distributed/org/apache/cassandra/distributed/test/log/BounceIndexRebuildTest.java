...
public class BounceIndexRebuildTest extends TestBaseImpl
{
    ...
    {
        try (Cluster cluster = init(builder().withNodes(1).withConfig(config -> config.with(NETWORK, GOSSIP)).start()))
        {
            ...
            cluster.schemaChange(withKeyspace("create index idx on %s.tbl (x) using 'sai'"));
