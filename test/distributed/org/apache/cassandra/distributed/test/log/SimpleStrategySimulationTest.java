...
{
    private static final Logger logger = LoggerFactory.getLogger(SimpleStrategySimulationTest.class);
    private long seed;
    private Random rng;
    static
    {
    ...
    public void simulateSimple() throws Throwable
    {
        for (int concurrency : new int[]{ 1, 3, 5 })
        {
            for (int rf : new int[]{ 2, 3, 5 })
            {
                ...
                {
                    simulate(seed, rng, 50, 0, new SimpleReplicationFactor(rf, trans), concurrency);
                }
            }
        }
    }
