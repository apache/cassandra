...
public class DecommissionTest extends TestBaseImpl
{
    ...
    public void testAbortingDecommissionRestreams() throws Exception
    {
        ...
        // Only install on the first classloader handed out for node2: a restarted instance gets a
        // fresh classloader, so any static "already failed once" state would be reset and the
        // resumed decommission would fail again. On this branch the initializer is a
        // BiConsumer<ClassLoader, Integer>, so there is no generation argument to key off.
        ...
        try (Cluster cluster = builder().withNodes(4)
                                        ...
                                        .withInstanceInitializer((cl, num) -> {
                                            if (num == 2 && streamHintsInstalled.compareAndSet(false, true))
                                        ...
                                        .start())
        {
            ...
            List<Murmur3Partitioner.LongToken> tokens = Collections.singletonList(new Murmur3Partitioner.LongToken(Long.parseLong(ClusterUtils.getLocalToken(leavingNode))));
    ...
    public static class BB
    {
        ...
        public static Future streamHints(@SuperCall Callable<Future> zuper)
        {
            ...
            return Futures.immediateFailedFuture(new IOException("failing hints so that decomm fails at last moment possible"));
        }
    }
}
