...
public class SimpleReadWriteTest extends TestBaseImpl
{
    ...
    public void coordinatorWriteTest() throws Throwable
    {
        try (Cluster cluster = init(builder().withNodes(3).start()))
        {
            ...
            String insertQuery = withKeyspace("INSERT INTO %s.tbl (pk, ck, v) VALUES (?, ?, ?)");
            String selectQuery = withKeyspace("SELECT * FROM %s.tbl WHERE pk = 1");
            Object[] row = row(1, 1, 1);

            cluster.coordinator(1).execute(insertQuery, ConsistencyLevel.QUORUM, row);

            int writtenReplicas = 0;
            for (int i = 1; i <= 3; i++)
            {
                Object[][] rows = cluster.get(i).executeInternal(selectQuery);
                if (rows.length == 1 && Arrays.equals(row, rows[0]))
                    writtenReplicas++;
            }
            Assert.assertTrue("The row should have been written in at least two replicas", writtenReplicas >= 2);

            assertRows(cluster.coordinator(1).execute(selectQuery, ConsistencyLevel.QUORUM), row);
        }
    }
