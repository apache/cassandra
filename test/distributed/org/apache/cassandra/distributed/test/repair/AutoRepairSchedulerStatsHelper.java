        ...
        for (String keyspace : keyspaces)
        {
            for (String table : Arrays.asList(TABLE1, TABLE2))
            {
                ...
                for (SSTableReader sstable : sstables)
                {
                    ...
                    {
                        List<AbstractBounds<PartitionPosition>> bounds = BigTableScanner.makeBounds(sstable, Collections.singleton(tokenRange));
                        ISSTableScanner rangeScanner = BigTableScanner.getScanner(sstable, Collections.singleton(tokenRange));
