...
public class HintsServiceMetricsTest extends TestBaseImpl
{
    ...
    public void testHintsServiceMetrics() throws Exception
    {
        ...
        try (Cluster cluster = builder().withNodes(3)
                                        ...
                                        .start())
        {
            // setup a message filter to drop some of the hint request messages
            AtomicInteger hintsNode1 = new AtomicInteger();
            AtomicInteger hintsNode2 = new AtomicInteger();
            cluster.filters()
                   .verbs(Verb.HINT_REQ.id)
                   .messagesMatching((from, to, message) ->
                                     (to == 2 && hintsNode1.incrementAndGet() <= NUM_TIMEOUTS_PER_NODE) ||
                                     (to == 3 && hintsNode2.incrementAndGet() <= NUM_TIMEOUTS_PER_NODE))
                   .drop();
            ...
            // write the first half of the rows with the second node down, so some hints will be created for that node
            node2.shutdown().get();
            for (int i = 0; i < NUM_ROWS / 2; i++)
                coordinator.execute(withKeyspace("INSERT INTO %s.t (k, v) VALUES (?, ?)"), QUORUM, i, i);
            node2.startup();

            // write the second half of the rows with the third node down, so some hints will be created for that node
            node3.shutdown().get();
            for (int i = NUM_ROWS / 2; i < NUM_ROWS; i++)
                coordinator.execute(withKeyspace("INSERT INTO %s.t (k, v) VALUES (?, ?)"), QUORUM, i, i);
            node3.startup();

            // wait until all the hints have been successfully applied to the nodes that have been down
            await().atMost(5, MINUTES)
                   .pollDelay(1, SECONDS)
                   .until(() -> countRows(node2) == NUM_ROWS && countRows(node3) == NUM_ROWS);

            // pause hints delivery so it doesn't interfere with the assertions below
            cluster.forEach(node -> node.runOnInstance(() -> HintsService.instance.pauseDispatch()));
            Thread.sleep(1000);

            // Verify the metrics for the coordinator node, which is the only one actually sending hints.
            // The hint delivery errors that we have injected should have made the service to try to send them again.
            // This retries are done periodically and in pages, so the retries can send again some of the hints that
            ...
            assertThat(countEndpointDelays(node1, node2) + countEndpointDelays(node1, node3)).isEqualTo(numGlobalDelays);
