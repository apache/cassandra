...
/**
 * Test Zstd compression levels 9-19 compared against level 8 baseline
 * CloudKit currently uses Zstd level 8 for L1+ compactions
 */
public class ZstdCompressionTest
{
    private static final Logger logger = LoggerFactory.getLogger(ZstdCompressionTest.class);
...
//    private static final String SSTABLE_PATH = "/Users/minalkyada/Desktop/p01_clouddb/custom_zone_records-5fb29180edd8300084c8712786214e10";
     ...
     */
    private CompressionResult testZstdCompressionWithIterations(int compressionLevel, int iterations) throws Exception
    {
    ...
    @Test
    public void testZstdCompressionLevels9To19() throws Exception
    {
        ...
        // Capture baseline with Zstd level 8
        CompressionResult baseline = testZstdCompressionWithIterations(8, ITERATIONS);

        // Test compression levels 9 through 19
        List<CompressionResult> results = new ArrayList<>();
        for (int level = 9; level <= 19; level++)
        {
            CompressionResult result = testZstdCompressionWithIterations(level, ITERATIONS);
            results.add(result);
            Thread.sleep(2000);
        }
    ...
    private void printResultsSummary(List<CompressionResult> results, CompressionResult baseline)
    {
        logger.info("");
        logger.info("================================================================================");
        logger.info("         ZSTD COMPRESSION RESULTS (AVERAGED OVER {} ITERATIONS)", baseline.iterations);
        logger.info("================================================================================");
        logger.info("");
        logger.info("BASELINE (Zstd Level 8 - CloudKit Current):");
