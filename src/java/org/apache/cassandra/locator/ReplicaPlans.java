...
public class ReplicaPlans
{
    ...
    public static ReplicaPlan.ForTokenRead forFollowupRead(Keyspace keyspace, Token token, ConsistencyLevel consistencyLevel, SpeculativeRetryPolicy retry, Collection<InetAddressAndPort> contacted)
    {
        AbstractReplicationStrategy replicationStrategy = keyspace.getReplicationStrategy();
        EndpointsForToken candidates = candidatesForRead(consistencyLevel, ReplicaLayout.forTokenReadLiveSorted(replicationStrategy, token).natural());
        EndpointsForToken contacts = candidates.filter(candidate -> contacted.contains(candidate.endpoint()));
