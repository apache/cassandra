...
public abstract class ReadCommand extends AbstractReadQuery
{
    ...
    private boolean shouldTrackSize(DataStorageSpec.LongBytesBound warnThreshold, DataStorageSpec.LongBytesBound failThreshold)
    {
        return trackWarnings
               && !SchemaConstants.isSystemKeyspace(metadata().keyspace)
               && !(warnThreshold.toBytes() == 0 && failThreshold.toBytes() == 0);
    }
    ...
    private UnfilteredPartitionIterator withQuerySizeTracking(UnfilteredPartitionIterator iterator)
    {
        ...
        class QuerySizeTracking extends Transformation<UnfilteredRowIterator>
        {
            ...
            private void addSize(long size)
            {
                ...
                {
                    String msg = String.format("Query %s attempted to read %d bytes but max allowed is %d; query aborted  (see local_read_size_fail_threshold)",
                                               ReadCommand.this.toCQLString(), this.sizeInBytes, failBytes);
                    Tracing.trace(msg);
