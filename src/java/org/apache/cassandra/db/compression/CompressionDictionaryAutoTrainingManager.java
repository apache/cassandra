...
public class CompressionDictionaryAutoTrainingManager implements AutoCloseable
{
    ...
    synchronized void start(long initialDelay, long interval, TimeUnit unit)
    {
        ...
        logger.info("Auto-training manager started, checking every {} {} with initial delay {} {}.",
                    interval, unit, initialDelay, unit);
    }
                                          ...
                                          CompressionDictionaryTrainingConfig config)
    {
        ...
        try (trainer; CompressionRatioEvaluator evaluator = createEvaluator(cfs, refViewFragment, trainer, config))
        {
            ...
            // A usable ratio is strictly positive. computeSampleCompressionRatio yields -1.0
            // (NO_COMPRESSION_RATIO) when nothing was sampled, and -1.0 is finite, so unguarded it
            // inverts the comparison below: a -1.0 candidate - one that measured nothing at all -
            // computes as +300% improvement and is promoted over a dictionary that works.
            if (compressionRatioLatestDict <= 0 || compressionRatioAutoTrainedDict <= 0)
            {
                logger.debug("No usable compression ratio for table {}.{} (baseline={}, candidate={}); " +
                             "nothing was sampled, so there is nothing to compare and the candidate " +
                             "is not promoted.",
                             cfs.getKeyspaceName(), cfs.getTableName(),
                             compressionRatioLatestDict, compressionRatioAutoTrainedDict);
                return false;
            }
