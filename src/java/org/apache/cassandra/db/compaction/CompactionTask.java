...
public class CompactionTask extends AbstractCompactionTask
{
    ...
    {
        long gcBefore = this.gcBefore;
        TableMetadata metadata = cfs.metadata();
        if (metadata.isAccordEnabled())
        {
            Invariants.expect(gcBefore <= 0);
            ...
            gcBefore = node.durableBefore().foldlWithDefault(ranges, (e, v) -> e == null ? NO_GC : Math.min(e.universal.hlc(), v), null, Long.MAX_VALUE);
            CommandStores.StoreSelector selector = StoreFinder.selector(ranges, Long.MIN_VALUE, Long.MAX_VALUE);
            gcBefore = node.commandStores().mapReduceUnsafe(selector, commandStore -> commandStore.unsafeGetRedundantBefore().foldl(ranges, (bs, v) -> bs.maxBound(LOCALLY_APPLIED).hlc(), Long.MAX_VALUE), Math::min, gcBefore);
            if (gcBefore == Long.MAX_VALUE)
                gcBefore = NO_GC;
        }
        return getCompactionController(toCompact, gcBefore);
    }
