...
through $f-1$. Because of this UCS will produce one compaction candidate per level of the compaction hierarchy, randomly
selecting among the overlap buckets, and out of the levels having candidates, it selects one randomly.

Because the size of individual sstables is close to equal among the levels, this should give each level close to the
same division of the processing resources. Because the compaction size is limited, each compaction should be quick
enough to not risk being unable to process new sstables on L0 for a long period of time.
