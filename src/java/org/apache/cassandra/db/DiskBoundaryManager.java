    ...
    private static DiskBoundaries getDiskBoundaryValue(ColumnFamilyStore cfs)
    {
        ...
        do
        {
                ...
                && !StorageService.isReplacingSameAddress()) // When replacing same address, the node marks itself as UN locally
            {
                ...
                localRanges = ReplicatedRanges.asRanges(tmd.getPendingRanges(cfs.keyspace.getName(), FBUtilities.getBroadcastAddressAndPort()));
            }
            else
            {
                ...
                localRanges = ReplicatedRanges.asRanges(cfs.keyspace.getReplicationStrategy().getAddressRanges(tmd.cloneAfterAllSettled()).get(FBUtilities.getBroadcastAddressAndPort()));
            }
