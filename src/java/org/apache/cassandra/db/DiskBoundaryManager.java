    ...
    public DiskBoundaries getDiskBoundaries(ColumnFamilyStore cfs, TableMetadata metadata)
    {
        ...
        if (diskBoundaries == null || diskBoundaries.isOutOfDate())
        {
            synchronized (this)
            {
                if (diskBoundaries == null || diskBoundaries.isOutOfDate())
                {
                    if (logger.isTraceEnabled()) logger.trace("Refreshing disk boundary cache for {}.{}", cfs.getKeyspaceName(), cfs.getTableName());
                    DiskBoundaries oldBoundaries = diskBoundaries;
                    diskBoundaries = getDiskBoundaryValue(cfs, metadata.partitioner);
                    if (logger.isTraceEnabled()) logger.trace("Updating boundaries from {} to {} for {}.{}", oldBoundaries, diskBoundaries, cfs.getKeyspaceName(), cfs.getTableName());
                }
            }
        }
