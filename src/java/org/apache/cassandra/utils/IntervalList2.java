...
public class IntervalList2<C extends Comparable<? super C>, D, I extends Interval<C, D>> implements Iterable<I> {
    @SuppressWarnings({ "rawtypes", "unchecked" })
    private static final IntervalList2 EMPTY_TREE = new IntervalList2(Collections.emptyList());
    ...
    protected IntervalList2(Collection<I> intervals) {
    ...
    public static <C extends Comparable<? super C>, D, I extends Interval<C, D>> IntervalList2<C, D, I> build(
            Collection<I> intervals) {
        if (intervals == null || intervals.isEmpty())
            return emptyTree();

        return new IntervalList2<C, D, I>(intervals);
    }
    ...
    public static <C extends Comparable<? super C>, D, I extends Interval<C, D>> IntervalList2<C, D, I> emptyTree() {
        return EMPTY_TREE;
    }
    ...
    public boolean equals(Object o) {
        if (!(o instanceof IntervalList2)) {
            return false;
        }

        @SuppressWarnings("rawtypes")
        IntervalList2 that = (IntervalList2) o;
    ...
    private int findIntervalsWithMinLessThanOrEqualToSearchIntervalMax(Interval<C, D> searchInterval) {
        ...
        if (index < 0) {
            ...
            // searchInterval.max is found in which case we need to find all the elements.
            while (index < intervalCount() && minOrderedIntervals.get(index).min.equals(searchInterval.max)) {
                index++;
            }
        }
    ...
    public static class Serializer<C extends Comparable<? super C>, D, I extends Interval<C, D>>
            implements IVersionedSerializer<IntervalList2<C, D, I>> {
        ...
        public void serialize(IntervalList2<C, D, I> it, DataOutputPlus out, int version) throws IOException {
        ...
        /**
         * Deserialize an IntervalList2 whose keys use the natural ordering. Use
         ...
         */
        public IntervalList2<C, D, I> deserialize(DataInputPlus in, int version) throws IOException {
            return deserialize(in, version, null);
        }

        public IntervalList2<C, D, I> deserialize(DataInputPlus in, int version, Comparator<C> comparator)
                throws IOException {
            try {
        ...
        public long serializedSize(IntervalList2<C, D, I> it, int version) {
