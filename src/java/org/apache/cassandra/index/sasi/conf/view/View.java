...
public class View implements Iterable<SSTableIndex> {
    ...
    private final IntervalList<Key, SSTableIndex, Interval<Key, SSTableIndex>> keyIntervalList;
    ...
    public View(ColumnIndex index, Collection<SSTableIndex> currentView, Collection<SSTableReader> oldSSTables,
        ...
        // Ensure oldSSTables and newIndexes are disjoint (in index redistribution case
        // the intersection can be non-empty).
        // also favor newIndexes over currentView in case an SSTable has been re-opened
        // (also occurs during redistribution)
        // See CASSANDRA-14055
        ...
        this.keyIntervalList = IntervalList.build(keyIntervals);

        if (keyIntervalList.intervalCount() != termTree.intervalCount())
            throw new IllegalStateException(
                    String.format("mismatched sizes for intervals tree for keys vs terms: %d != %d",
                            keyIntervalList.intervalCount(), termTree.intervalCount()));
    }
    ...
    public List<SSTableIndex> match(ByteBuffer minKey, ByteBuffer maxKey) {
        return keyIntervalList.search(
                Interval.create(new Key(minKey, keyValidator), new Key(maxKey, keyValidator), (SSTableIndex) null));
    }
    ...
    /**
     * This is required since IntervalList doesn't support custom Comparator
     * implementations and relied on items to be comparable which "raw" keys are
     * not.
     */
