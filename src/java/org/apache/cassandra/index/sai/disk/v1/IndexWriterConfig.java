...
public class IndexWriterConfig
{
    ...
    public static IndexWriterConfig fromOptions(AbstractType<?> type, Map<String, String> options)
    {
            ...
            options.get(SIMILARITY_FUNCTION) != null)
        {
            ...
            if (options.containsKey(MAXIMUM_NODE_CONNECTIONS))
            {
                ...
                {
                    throw new InvalidRequestException(String.format("Maximum number of connections %s is not a valid integer",
                                                                    options.get(MAXIMUM_NODE_CONNECTIONS)));
                }
                if (maximumNodeConnections <= 0 || maximumNodeConnections > MAXIMUM_MAXIMUM_NODE_CONNECTIONS)
                    throw new InvalidRequestException(String.format("Maximum number of connections cannot be <= 0 or > %s, was %s", MAXIMUM_MAXIMUM_NODE_CONNECTIONS, maximumNodeConnections));
            }
            if (options.containsKey(CONSTRUCTION_BEAM_WIDTH))
            {
                ...
                {
                    throw new InvalidRequestException(String.format("Construction beam width %s is not a valid integer",
                                                                    options.get(CONSTRUCTION_BEAM_WIDTH)));
                }
                if (queueSize <= 0 || queueSize > MAXIMUM_CONSTRUCTION_BEAM_WIDTH)
                    throw new InvalidRequestException(String.format("Construction beam width cannot be <= 0 or > %s, was %s", MAXIMUM_CONSTRUCTION_BEAM_WIDTH, queueSize));
            }
            if (options.containsKey(SIMILARITY_FUNCTION))
            {
                ...
                {
                    throw new InvalidRequestException(String.format("Similarity function %s was not recognized. Valid values are: %s",
                                                                    option, validSimilarityFunctions));
                }
    ...
    {
        return new IndexWriterConfig(DEFAULT_MAXIMUM_NODE_CONNECTIONS,
