...
public class CassandraOnDiskHnsw implements AutoCloseable
{
    ...
    public CassandraOnDiskHnsw(SegmentMetadata.ComponentMetadataMap componentMetadatas, PerColumnIndexFiles indexFiles, IndexContext context) throws IOException
    {
        ...
        try (var vectors = new OnDiskVectorLoader(indexFiles.vectors(), vectorsSegmentOffset))
        {
            vectorDimension = vectors.dimension();
            vectorCache = VectorCache.load(hnsw.getView(), vectors, CassandraRelevantProperties.SAI_VECTOR_SEARCH_VECTOR_CACHE_BYTES.getInt());
        }
    }
    ...
    public ReorderingPostingList search(float[] queryVector, int topK, Bits acceptBits, int vistLimit, QueryContext context)
    {
        ...
        NeighborQueue queue;
        try (var vectors = vectorsSupplier.apply(context); var view = hnsw.getView())
        {
    ...
    class OnDiskVectorLoader implements AutoCloseable
    {
        protected final FileHandle fileHandle;
        protected final RandomAccessReader reader;
        protected final long segmentOffset;
        protected final int dimension;
        protected final int size;
        protected final float[] vector;

        public OnDiskVectorLoader(FileHandle fh, long segmentOffset)
        {
            this.fileHandle = fh;
            try
            {
            ...
            {
                throw new RuntimeException("Error initializing OnDiskVectorLoader at segment offset" + segmentOffset, e);
            }
        }
    ...
    class OnDiskVectors extends OnDiskVectorLoader implements RandomAccessVectorValues<float[]>
    {
        private final QueryContext queryContext;

        public OnDiskVectors(FileHandle fh, long segmentOffset, QueryContext queryContext)
        {
            super(fh, segmentOffset);
        ...
        {
            queryContext.vectorContext().hnswVectorsAccessed++;
            var cached = vectorCache.get(i);
            if (cached != null)
            {
                queryContext.vectorContext().hnswVectorCacheHits++;
                return cached;
            }
