...
public class OrderByProcessor
{
    ...
    private final Set<PrimaryKey> tombstonesSeen;
    private final IndexContext indexContext;

    public OrderByProcessor(ReadCommand command, StorageAttachedIndexQueryPlan queryPlan, IndexContext indexContext)
    {
        this.command = command;
        this.queryPlan = queryPlan;
        this.limit = command.limits().count();
        this.indexContext = indexContext;
        this.tombstonesSeen = new HashSet<>();
    }
        ...
        while (partitions.hasNext())
        {
            try (R partition = partitions.next())
            {
                ...
                while (partition.hasNext())
                {
                    ...
                    if (!row.hasLiveData(command.nowInSec(), true))
                        tombstonesSeen.add(indexContext.keyFactory().create(key, row.clustering()));
    ...
    public Set<PrimaryKey> getTombstoneTracker()
    {
        return tombstonesSeen;
    }
}
