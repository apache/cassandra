...
public class ClientMetricsManager
{
    ...
    public static ClientSessionMetrics getSessionMetrics(
    ...
    {
        String key = String.join(",", clientService, tenancy, tier, driverName, enforcementLevelString, isDriverSupportedString, isAuthenticated);
        return sessionMetrics.computeIfAbsent(key, k -> new ClientSessionMetrics(
            clientService,
            tenancy,
            tier,
            driverName,
            enforcementLevelString,
            isDriverSupportedString,
            isAuthenticated));
    }
