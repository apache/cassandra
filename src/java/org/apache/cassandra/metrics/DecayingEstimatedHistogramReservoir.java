...
public class DecayingEstimatedHistogramReservoir implements SnapshottingReservoir
{
    ...
    private DecayingBuckets rescale(long now)
    {
        ...
        {
            long newValue = Math.round(decayingBuckets.buckets.get(i) / rescaleFactor);
    ...
    {
        return (now - decayingBuckets.decayLandmark) > LANDMARK_RESET_INTERVAL_IN_NS;
    }
    ...
    public void rebase(EstimatedHistogramReservoirSnapshot snapshot)
    {
        ...
        for (int i = 0; i < size(); i++)
        {
            ...
            newDecayingBuckets.buckets.set(stripedIndex(i, 0), snapshot.decayingBuckets[i]);
    ...
    private class DecayingBuckets
    {
        ...
        public DecayingBuckets(long decayLandmark)
        {
            ...
            this.buckets = new AtomicLongArray((bucketOffsets.length + 1) * nStripes);
        }
