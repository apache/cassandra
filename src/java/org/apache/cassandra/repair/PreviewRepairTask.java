...
public class PreviewRepairTask extends AbstractRepairTask
{
    ...
    {
        Future<CoordinatedRepairResult> f = runRepair(coordinator.state.id, false, executor, commonRanges, coordinator.neighborsAndRanges.excludedDeadParticipants, coordinator.getColumnFamilyNames().toArray(new String[0]));
        return f.map(result -> {
            if (result.hasFailed())
                return result;
