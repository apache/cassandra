...
public class AutoRepairUtils
{
    ...
    "UPDATE %s.%s SET %s= ?, %s = ? WHERE %s = ? AND %s = ? IF EXISTS"
    , SchemaConstants.DISTRIBUTED_KEYSPACE_NAME, SystemDistributedKeyspace.AUTO_REPAIR_HISTORY, COL_REPAIR_START_TS, COL_REPAIR_TURN,
    ...
    @VisibleForTesting
    protected static TreeSet<UUID> getHostIdsInCurrentRing(RepairType repairType, Collection<InetAddressAndPort> allNodesInRing)
    {
    ...
    {
        Collection<InetAddressAndPort> allNodesInRing = StorageService.instance.getTokenMetadata().getAllEndpoints();
    ...
    private static Map<InetAddressAndPort, UUID> getBroadcastAddressToHostIdMap(Set<UUID> hosts)
    {
        ...
        {
            InetAddressAndPort broadcastAddress = StorageService.instance.getTokenMetadata().getEndpointForHostId(hostId);
                                                       ...
                                                       Map<AbstractReplicationStrategy, List<String>> replicationStrategies)
    {
        ...
        InetAddressAndPort eligibleBroadcastAddress = StorageService.instance.getTokenMetadata().getEndpointForHostId(eligibleHistory.hostId);
            ...
            for (Replica replica : rangesAtEndpoint)
            {
                ...
                for (InetAddressAndPort inetAddressAndPort : endpointsForRange.endpoints())
                {
                    ...
                    if (hostId != null)
                    {
                        ...
                        InetAddressAndPort myBroadcastAddress = StorageService.instance.getTokenMetadata().getEndpointForHostId(eligibleHistory.hostId);
            ...
            if (currentRepairStatus == null || parallelRepairNumber > currentRepairStatus.hostIdsWithOnGoingRepair.size())
            {
                ...
                if (nodeToBeRepaired != null)
                {
                    ...
                    logger.info("Next node to be repaired for repair type {}: {} ({})", repairType,
                                StorageService.instance.getTokenMetadata().getEndpointForHostId(nodeToBeRepaired.hostId),
                                 nodeToBeRepaired);
                }
    ...
    public static boolean runBootstrapRepair() throws InterruptedException
    {
        ...
        Future<?> future = executor.submit(() -> {
            ...
            {
                AutoRepairMetrics bootstrapRepairMetrics = AutoRepairMetricsManager.getMetrics(RepairType.BOOTSTRAP);
