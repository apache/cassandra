...
public class FixedSplitTokenRangeSplitter implements IAutoRepairTokenRangeSplitter
{
     ...
     */
    public static final int DEFAULT_NUMBER_OF_SUBRANGES = 16;
    ...
    public FixedSplitTokenRangeSplitter(AutoRepairConfig.RepairType repairType, Map<String, String> parameters)
    {
        ...
        // FIXME: this approach does not allow modifying the number of subranges dynamically
        numberOfSubranges = Integer.parseInt(parameters.getOrDefault(NUMBER_OF_SUBRANGES, Integer.toString(Math.min(DEFAULT_NUMBER_OF_SUBRANGES, Math.max(1, 256 / DatabaseDescriptor.getNumTokens())))));
    }
    ...
    private KeyspaceRepairAssignments getRepairAssignmentsForKeyspace(boolean primaryRangeOnly, int priority, KeyspaceRepairPlan repairPlan)
    {
        ...
        Collection<Range<Token>> tokens = StorageService.instance.getPrimaryRangesForEndpoint(keyspaceName, repairPlan.getEndpoint());
        if (!primaryRangeOnly)
        {
