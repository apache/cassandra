...
public class PaxosPrepare extends PaxosRequestCallback<PaxosPrepare.Response> implements PaxosPrepareRefresh.Callbacks, Paxos.Async<PaxosPrepare.Status>
{
    ...
    private static <R extends AbstractRequest<R>> void startTracked(PaxosPrepare prepare, Participants participants, Message<R> send, BiFunction<R, RequestTime, Future<Response>> selfHandler)
    {
        ...
        if (summaryIndex < summaryHostIds.length)
            summaryHostIds = Arrays.copyOf(summaryHostIds, summaryIndex);
    ...
    static Replica selectDataNode(Participants participants, InetAddressAndPort local)
    {
        ...
        if (localReplica != null && localReplica.isFull() && !participants.electorate.isPending(local))
            return localReplica;
