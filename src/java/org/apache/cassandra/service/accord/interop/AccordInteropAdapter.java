...
public class AccordInteropAdapter extends AbstractTxnAdapter
{
    ...
    {
        if (applyKind == Minimal && doInteropPersist(node, all, executes, route, txnId, txn, executeAt, deps, writes, result, callback))
            return;

        if (callback != null) callback.accept(result, null);
        new PersistTxn(node, all, txnId, route, txn, executeAt, deps, writes, result)
            .start(Apply.FACTORY, applyKind, all, executes, writes, result);
    }
    ...
    private static boolean doInteropPersist(Node node, Topologies all, Topologies executes, FullRoute<?> route, TxnId txnId, Txn txn, Timestamp executeAt, Deps deps, Writes writes, Result result, BiConsumer<? super Result, Throwable> callback)
    {
        ...
        new AccordInteropPersist(node, all, txnId, route, txn, executeAt, deps, writes, result, consistencyLevel, callback)
            .start(AccordInteropApply.FACTORY, Minimal, all, executes, writes, result);
        return true;
    }
}
