...
public abstract class AccordRoutingKey extends AccordRoutableKey implements RoutingKey
{
    ...
    public static class KeyspaceSplitter implements ShardDistributor
    {
        ...
        public List<Ranges> split(Ranges ranges)
        {
            ...
            for (Map.Entry<String, List<Range>> e : byKeyspace.entrySet())
            {
                List<Ranges> splits = subSplitter.split(Ranges.ofSortedAndDeoverlapped(e.getValue().toArray(new Range[0])));
                int i = 0;
                for (Ranges split : splits)
                {
                    if (i == results.size())
