...
public class AccordKeyspace
{
                                                                         ...
                                                                         Function<V, ByteBuffer> serializeVal)
    {
        if (map.wasCleared())
        {
            if (!map.hasAdditions())
            {
                builder.addComplexDeletion(column, DeletionTime.buildUnsafeWithUnsignedInteger(timestamp, nowInSec));
                return;
            }
            else
                builder.addComplexDeletion(column, DeletionTime.buildUnsafeWithUnsignedInteger(timestamp - 1, nowInSec));
        }
