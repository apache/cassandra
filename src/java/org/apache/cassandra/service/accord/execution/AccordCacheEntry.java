...
public class AccordCacheEntry<K, V, S extends SafeState<V> & SaferState<K, V, S>> extends IntrusiveLinkedListNode
{
    ...
    /**
     * Either a SafeTask, AccordCacheEntryMiniQueue or AccordCacheEntryQueue.
     * See {@link #ensureQueue()} to understand how to interpret a SafeTask as a (potentially empty) queue
     */
    ...
    // a task upgrades all entries to fifo when first prepared to run, to ensure it is correctly ordered wrt other tasks
    ...
    private RunnableStatus add(SafeTask<?> task, TriFunction<AccordCacheEntryQueue, AccordCacheEntry<?, ?, ?>, SafeTask<?>, RunnableStatus> add)
    {
        ...
        boolean isLoaded = isLoaded(); // an unloaded entry does not maintain/notify runnable statuses, since nothing is runnable
    ...
    /**
     * On lock we remove ourselves from the priority/fifo queues and notify the new head, unless lockMode is HOLD_QUEUE
     */
    ...
    public static class UniqueSave
    {
        ...
        static void notify(@Nullable UniqueSave identity, @Nullable Throwable failure)
        {
            ...
            List<BiConsumer<Void, Throwable>> notify = identity.notify;
            if (notify == null)
                return;
