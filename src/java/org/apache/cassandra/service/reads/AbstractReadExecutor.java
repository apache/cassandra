...
public abstract class AbstractReadExecutor
{
    ...
    AbstractReadExecutor(ReadCoordinator coordinator, ColumnFamilyStore cfs, ReadCommand command, ReplicaPlan.ForTokenRead replicaPlan, int initialDataRequestCount, long queryStartNanoTime)
    {
        ...
        this.readRepair = ReadRepair.create(coordinator, command, this.replicaPlan, queryStartNanoTime);
        this.digestResolver = new DigestResolver<>(coordinator, command, this.replicaPlan, queryStartNanoTime);
    ...
    private void makeRequests(ReadCommand readCommand, Iterable<Replica> replicas)
    {
        ...
        for (Replica replica: replicas)
        {
            ...
            coordinator.sendReadCommand(message, endpoint, handler);
        }
                                                       ...
                                                       long queryStartNanoTime) throws UnavailableException
    {
        ...
        if (retry.equals(NeverSpeculativeRetryPolicy.INSTANCE) || consistencyLevel == ConsistencyLevel.EACH_QUORUM)
            return new NeverSpeculatingReadExecutor(coordinator, cfs, command, replicaPlan, queryStartNanoTime, false);
        ...
        if (replicaPlan.contacts().size() == replicaPlan.readCandidates().size())
        {
            ...
            return new NeverSpeculatingReadExecutor(coordinator, cfs, command, replicaPlan, queryStartNanoTime, recordFailedSpeculation);
        }

        if (retry.equals(AlwaysSpeculativeRetryPolicy.INSTANCE))
            return new AlwaysSpeculatingReadExecutor(coordinator, cfs, command, replicaPlan, queryStartNanoTime);
        else // PERCENTILE or CUSTOM.
            return new SpeculatingReadExecutor(coordinator, cfs, command, replicaPlan, queryStartNanoTime);
    }
    ...
    public static class NeverSpeculatingReadExecutor extends AbstractReadExecutor
    {
        ...
        public NeverSpeculatingReadExecutor(ReadCoordinator coordinator, ColumnFamilyStore cfs, ReadCommand command, ReplicaPlan.ForTokenRead replicaPlan, long queryStartNanoTime, boolean logFailedSpeculation)
        {
            super(coordinator, cfs, command, replicaPlan, 1, queryStartNanoTime);
    ...
    static class SpeculatingReadExecutor extends AbstractReadExecutor
    {
                                       ...
                                       long queryStartNanoTime)
        {
            ...
            super(coordinator, cfs, command, replicaPlan, replicaPlan.readQuorum() < replicaPlan.contacts().size() ? 2 : 1, queryStartNanoTime);
        }
    ...
    private static class AlwaysSpeculatingReadExecutor extends AbstractReadExecutor
    {
                                             ...
                                             long queryStartNanoTime)
        {
            ...
            super(coordinator, cfs, command, replicaPlan, replicaPlan.contacts().size() > 1 ? 2 : 1, queryStartNanoTime);
        }
