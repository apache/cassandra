...
public class DataResolver<E extends Endpoints<E>, L extends ReplicaPlan<E, L>> extends ResponseResolver<E, L>
{
    private final boolean enforceStrictLiveness;

    public DataResolver(ReadCommand command, P replicaPlan, ReadRepair<E, L> readRepair, long queryStartNanoTime)
    {
    ...
    public PartitionIterator resolve()
    {
        ...
        E replicas = replicaPlan.all().keep(transform(messages, msg -> msg.from));
        ...
        UnfilteredPartitionIterator merged = mergeWithShortReadProtection(iters,
                                                                          replicaPlan.withSelected(replicas),
                                                                          mergedResultCounter,
                                                                          repairedDataTracker);
                                                                     ...
                                                                     RepairedDataTracker repairedDataTracker)
    {
        ...
        if (!command.limits().isUnlimited())
            for (int i = 0; i < results.size(); i++)
                results.set(i, ShortReadProtection.extend(sources.contact().get(i), results.get(i), command, mergedResultCounter, queryStartNanoTime, enforceStrictLiveness));
