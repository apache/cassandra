    ...
    private void validateIndexTarget(TableMetadata table, IndexTarget target)
    {
        ...
        if (column.type.isFrozenCollection() && target.type != Type.FULL)
            throw ire("Cannot create %s() index on frozen column %s. Frozen collections only support indexes on the " +
                      "entire data structure due to immutability constraints of being frozen, wrap your frozen column " +
                      "with the full() target type to index properly", target.type, column);
