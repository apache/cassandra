...
Because the legacy mechanism polices `System.*` at runtime on the secured UDF threads, the byte-code
verifier used on this path is the *base* verifier (which does not statically block the `System` methods)
when UDF threads are enabled. Synchronous UDFs do not run on a secured thread, so they use the sandbox
verifier even under a `SecurityManager`, unless the `allow_extra_insecure_udfs` opt-in applies (see
...
the **default-setting methods** `Locale.setDefault`, `TimeZone.setDefault`, and `SimpleTimeZone.setDefault`
(which required a property-write permission under the `SecurityManager`), and the system-logger back door
`System$LoggerFinder.getLoggerFinder` (which the empty UDF policy denied under the `SecurityManager` via
`RuntimePermission("loggerFinder")`), so the SecurityManager-free path does not expose a capability the
legacy path withheld.
...
- every call whose owner is `java.lang.ClassLoader` (the class is denied outright by the verifier, so
  `loadClass`, `getResource*`, `getSystem*`, the assertion-status setters and every other `ClassLoader`
  method are unreachable);
...
`auto` resolves to the `SecurityManager` path only when `Runtime.version().feature() < 24`
(`ThreadAwareSecurityManager.useSecurityManager()`, which re-reads the property and re-evaluates the
version on every call, i.e. at `install()` and at each `CREATE FUNCTION`). When the SecurityManager-free
path is in effect, `install()` installs no `SecurityManager` or `Policy` (it only logs the mechanism and
runs the logging-framework startup hook); the class loader and creation-time verification carry the
sandbox. An invalid value raises a `ConfigurationException`.
...
- When a `SecurityManager` is in use *and* UDF threads are enabled, the **base** verifier is selected —
  `System.*` is policed by the `SecurityManager` at runtime on the secured UDF thread.
- When UDF threads are disabled *and* `allow_extra_insecure_udfs` is `true` (the explicit insecure opt-in
  described next), the **base** verifier is selected regardless of mechanism.
- Otherwise the **sandbox** verifier (with the `System.*` block) is selected. This includes synchronous
  UDFs under a `SecurityManager`: they do not run on a secured thread, so the static block applies (the
  class loader's `disallowedPatternsSyncUDF` additionally denies `java.lang.System` outright there).
...
away from the `SecurityManager`. The one case that changes is a `SecurityManager` with UDF threads
disabled and no insecure opt-in: `java.lang.System` was already unresolvable there (the class loader's
`disallowedPatternsSyncUDF`), but such UDFs run on the caller thread, where `checkPermission` checks
nothing beyond `setSecurityManager`, so the base verifier left the property-read aliases, the
`setDefault` methods and `LoggerFinder.getLoggerFinder` reachable. The sandbox verifier now rejects
those statically there too.
