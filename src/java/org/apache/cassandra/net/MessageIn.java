...
public class MessageIn<T>
{
    ...
    public static abstract class MessageInProcessor
    {
        ...
        public void process(ByteBuf buf) throws IOException
        {
            ByteBufDataInputPlus inputPlus = new ByteBufDataInputPlus(buf);
            while (true)
            {
                switch (state)
                {
                    case READ_PREFIX:
                        if (!readPrefix(buf, inputPlus))
                            return;
                        state = State.READ_IP_ADDRESS;
                        break;
                    case READ_IP_ADDRESS:
                        if (!readIpAddress(buf, inputPlus, messageHeader))
                            return;
                        state = State.READ_VERB;
                        break;
                    case READ_VERB:
                        if (!readVerb(buf, messageHeader))
                            return;
                        state = State.READ_PARAMETERS_SIZE;
                        break;
                    case READ_PARAMETERS_SIZE:
                        if (!readParametersSize(buf, messageHeader))
                            return;
                        state = State.READ_PARAMETERS_DATA;
                        break;
                    case READ_PARAMETERS_DATA:
                        if (!readParameterData(buf, inputPlus, messageHeader))
                            return;
                        state = State.READ_PAYLOAD_SIZE;
                        break;
                    case READ_PAYLOAD_SIZE:
                        if (!readPayloadSize(buf, messageHeader))
                            return;
                        state = State.READ_PAYLOAD;
                        break;
                    case READ_PAYLOAD:
                        if (!readPayload(buf, inputPlus, messageHeader))
                            return;
        ...
        boolean readPrefix(ByteBuf buf, ByteBufDataInputPlus inputPlus) throws IOException
        {
            if (buf.readableBytes() < MessageOut.MESSAGE_PREFIX_SIZE)
                return false;
        ...
        boolean readVerb(ByteBuf buf, MessageHeader messageHeader)
        {
            if (buf.readableBytes() < VERB_LENGTH)
                return false;
            messageHeader.verb = MessagingService.Verb.fromId(buf.readInt());
            return true;
        }

        abstract boolean readIpAddress(ByteBuf buf, ByteBufDataInputPlus inputPlus, MessageHeader messageHeader) throws IOException;

        abstract boolean readParametersSize(ByteBuf buf, MessageHeader messageHeader);

        abstract boolean readParameterData(ByteBuf buf, ByteBufDataInputPlus inputPlus, MessageHeader messageHeader) throws IOException;

        abstract boolean readPayloadSize(ByteBuf buf, MessageHeader messageHeader);

        boolean readPayload(ByteBuf buf, ByteBufDataInputPlus inputPlus, MessageHeader messageHeader) throws IOException
        {
            if (buf.readableBytes() < messageHeader.payloadSize)
                return false;
    ...
    static class MessageInProcessorAsOf40 extends MessageInProcessor
    {
        ...
        boolean readIpAddress(ByteBuf buf, ByteBufDataInputPlus inputPlus, MessageHeader messageHeader)
        {
            messageHeader.from = peer;
            return true;
        }

        boolean readParametersSize(ByteBuf buf, MessageHeader messageHeader)
        {
            // VIntCoding.readUnsignedVInt() returns -1 is it cannot read the length
            long length = VIntCoding.readUnsignedVInt(buf);
            if (length < 0)
                return false;
        ...
        boolean readParameterData(ByteBuf buf, ByteBufDataInputPlus inputPlus, MessageHeader messageHeader) throws IOException
        {
            if (messageHeader.parameterLength == 0)
                return true;

            if (buf.readableBytes() < messageHeader.parameterLength)
                return false;
        ...
        boolean readPayloadSize(ByteBuf buf, MessageHeader messageHeader)
        {
            long length = VIntCoding.readUnsignedVInt(buf);
            if (length < 0)
                return false;
    ...
    static class MessageInProcessorPre40 extends MessageInProcessor
    {
        ...
        boolean readIpAddress(ByteBuf buf, ByteBufDataInputPlus inputPlus, MessageHeader messageHeader) throws IOException
        {
            ...
            if (readableBytes < 1 || readableBytes < buf.getByte(buf.readerIndex()) + 1)
                return false;
        ...
        boolean readParametersSize(ByteBuf buf, MessageHeader messageHeader)
        {
            if (buf.readableBytes() < PARAMETERS_SIZE_LENGTH)
                return false;

            messageHeader.parameterLength = buf.readInt();
            return true;
        }
        ...
        boolean readPayloadSize(ByteBuf buf, MessageHeader messageHeader)
        {
            if (buf.readableBytes() < PAYLOAD_SIZE_LENGTH)
                return false;

            messageHeader.payloadSize = buf.readInt();
            return true;
        }
