...
public class BigTableWriterV3 extends SSTableWriter
{
    ...
    private final SequentialWriterOption writerOption = SequentialWriterOption.newBuilder()
                                                                              ...
                                                                              .hasMaxCompressedLength(false)
                                                                              .build();

    public BigTableWriterV3(Descriptor descriptor,
                            long keyCount,
                            long repairedAt,
    ...
    public SSTableReader openEarly()
    {
        ...
        IndexSummary indexSummary = iwriter.summary.build(metadata.get().partitioner, boundary);
    ...
    private SSTableReader openFinal(SSTableReader.OpenReason openReason)
    {
        ...
        IndexSummary indexSummary = iwriter.summary.build(this.metadata.get().partitioner);
        long indexFileLength = new File(descriptor.filenameFor(Component.PRIMARY_INDEX)).length();
        int dataBufferSize = optimizationStrategy.bufferSize(stats.estimatedPartitionSize.percentile(DatabaseDescriptor.getDiskOptimizationEstimatePercentile()));
        int indexBufferSize = optimizationStrategy.bufferSize(indexFileLength / indexSummary.size());
    ...
    class IndexWriter extends AbstractTransactional implements Transactional
    {
        ...
        IndexWriter(long keyCount)
        {
            ...
            summary = new IndexSummaryBuilder(keyCount, metadata.get().params.minIndexInterval, Downsampling.BASE_SAMPLING_LEVEL);
            bf = FilterFactory.getFilter(keyCount, metadata.get().params.bloomFilterFpChance);//, true, descriptor.version.hasOldBfHashOrder());
