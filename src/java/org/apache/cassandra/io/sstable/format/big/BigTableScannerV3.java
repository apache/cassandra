...
public class BigTableScannerV3 implements ISSTableScanner
{
    ...
    {
        return new BigTableScannerV3(sstable,
    ...
    {
        return new BigTableScannerV3(sstable, columns, dataRange, null, false, makeBounds(sstable, dataRange).iterator(), listener);
    }
    ...
    public static ISSTableScanner getScanner(SSTableReader sstable,
                                             ...
                                             SSTableReadsListener listener)
    {
        return new BigTableScannerV3(sstable, columns, dataRange, limiter, isForThrift, makeBounds(sstable, dataRange).iterator(), listener);
    }
    ...
    {
        return new BigTableScannerV3(sstable, ColumnFilter.all(sstable.metadata()), null, rangeIterator);
    }

    private BigTableScannerV3(SSTableReader sstable,
                              ColumnFilter columns,
                              RateLimiter limiter,
        ...
        try
        {
            ...
            while (!ifile.isEOF())
            {
                ...
                if (indexDecoratedKey.compareTo(currentRange.left) > 0 || currentRange.contains(indexDecoratedKey))
                {
                    ...
                    // removed sstable.descriptor.version
                ...
                return new LazilyInitializedUnfilteredRowIterator(currentKey)
                {
                    protected UnfilteredRowIterator initializeIterator()
                    {
                        ...
                        try
                        {
                            ...
                            return sstable.rowIterator(dfile, partitionKey(), currentEntry, filter.getSlices(BigTableScannerV3.this.metadata()), columns, filter.isReversed());
                        }
