...
/**
 * Writes the components of an entire-sstable stream to disk exactly as they arrive, and nothing else: every
 * component comes from the sender's manifest, and none is synthesised from the bytes that arrive.
 * <p>
 * Digest.crc32 in particular. A whole-sstable stream sends the sender's own copy like any other component. A
 * partial zero-copy stream cannot: its Data.db is byte ranges of a larger file that reach the socket by
 * {@code sendfile} without entering the sending process, so digesting them there would cost a second full read of
 * the data. Digesting them here as they are written would be free, but the result would be a checksum of the bytes
 * that arrived rather than of the bytes that were sent -- which is the one thing a digest exists to tell apart --
 * so it would make transfer corruption undetectable while reporting success from {@code nodetool verify}. Left
 * absent instead, {@code SortedTableVerifier} reports the component missing and verifies the sstable itself by
