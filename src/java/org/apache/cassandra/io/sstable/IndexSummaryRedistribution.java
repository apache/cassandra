...
public class IndexSummaryRedistribution extends CompactionInfo.Holder
{
                                                     ...
                                                     double totalReadsPerSec, long memoryPoolCapacity) throws IOException
    {
        ...
        for (Map.Entry<TableId, Set<SSTableReader>> entry : sstablesToCancelByTableId.entrySet()) {
            if (DatabaseDescriptor.isDaemonInitialized() && DatabaseDescriptor.getUnmarkCompactingIndexRedistributionInBatchEnabled())
                transactions.get(entry.getKey()).cancelInBatch(entry.getValue());
            else
