...
public final class Reflink
{
                                        ...
                                        long length, File directory)
    {
        ...
        try
        {
            srcFd = NativeLibrary.getfd(src);
            dstFd = NativeLibrary.getfd(dst);
        }
        catch (AssertionError | LinkageError e)
        {
            // NativeLibrary initialises its reflective FileChannel access on first use. An embedded or otherwise
            // non-standard launcher may omit the required --add-opens, in which case that initialisation first fails
            // with AssertionError and later attempts fail with NoClassDefFoundError. Extent sharing is optional;
            // preserve the documented byte-copy fallback rather than failing the split. Do not catch Error broadly:
            // VM failures such as OutOfMemoryError are not a reason to continue a large copy.
            logger.debug("Could not initialise native file-descriptor access; copying instead", e);
    ...
    /**
     * Remember that {@link NativeLibrary#getfd} could not read {@code sun.nio.ch.FileChannelImpl.fd}. Depending on
     * when {@code NativeLibrary} is initialised, missing
     * {@code --add-opens java.base/sun.nio.ch=ALL-UNNAMED} can either make it answer -1 or raise an
     * {@link AssertionError}/{@link LinkageError}; {@link #tryCloneRange} handles both. A channel from a non-default
     * {@code FileSystemProvider} can also answer -1. Either way it is a property of the JVM rather than of any
     * filesystem, so it is remembered once for the process and says which. Going through
