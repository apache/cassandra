...
public class SSTableExport
{
    ...
    private static void process(ISSTableScanner scanner, Stream<UnfilteredRowIterator> partitions, TableMetadata metadata) throws IOException
    {
        if (cmd.hasOption(DEBUG_OUTPUT_OPTION))
        {
            ...
            partitions.forEach(partition ->
            {
                ...
                partition.forEachRemaining(row ->
                {
                    ...
                    if (cmd.hasOption(ENUMERATE_TOMBSTONES_OPTION) && row.isRow())
        ...
        {
            JsonTransformer.toJsonLines(scanner, partitions, cmd.hasOption(RAW_TIMESTAMPS), cmd.hasOption(ENUMERATE_TOMBSTONES_OPTION), metadata, System.out);
        }
        else
        {
            JsonTransformer.toJson(scanner, partitions, cmd.hasOption(RAW_TIMESTAMPS), cmd.hasOption(ENUMERATE_TOMBSTONES_OPTION), metadata, System.out);
        }
    }
