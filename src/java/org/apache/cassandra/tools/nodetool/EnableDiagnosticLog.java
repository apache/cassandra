...
public class EnableDiagnosticLog extends NodeTool.NodeToolCmd
{
    ...
    @Option(title = "skip_persistent_log", name = { "--skip-persistent-log" }, description = "Skips persistent log from enablement.")
    private boolean skipPersistentLog = false;

    @Option(title = "skip_memory_log", name = { "--skip-memory-log" }, description = "Skips memory log from enablement.")
    private boolean skipMemoryLog = false;

    @Option(title = "use_node_configuration", name = { "--use-node-config" }, description = "When specified, it will take the configuration for persistent diagnostic logger from cassandra.yaml " +
                                                                                            "otherwise it will take configuration parameters for a logger from this command.")
    private boolean useNodeConfiguration = false;
    ...
    public void execute(NodeProbe probe)
    {
        ...
        if (!skipPersistentLog)
        {
            if (!skipMemoryLog)
            {
                if (useNodeConfiguration)
                {
                    probe.enableDiagnostics(true);
                }
                else
                {
                    probe.enableDiagnostics(false);
                    probe.enablePersistentDiagnosticLog(logger, Collections.emptyMap(), archiveRetries, bblocking, rollCycle, maxLogSize, maxQueueWeight, archiveCommand);
                }
            }
            else
            {
                if (useNodeConfiguration)
                    probe.enablePersistentDiagnosticLog();
                else
                    probe.enablePersistentDiagnosticLog(logger, Collections.emptyMap(), archiveRetries, bblocking, rollCycle, maxLogSize, maxQueueWeight, archiveCommand);
            }
        }
        else if (!skipMemoryLog)
        {
            probe.enableDiagnostics(false);
        }
    }
