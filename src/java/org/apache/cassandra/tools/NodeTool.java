...
public class NodeTool
{
    ...
    private static void configureCliLayout(CommandLine commandLine)
    {
        ...
        CliLayout layoutSysEnv = CassandraRelevantProperties.CASSANDRA_CLI_LAYOUT.getEnum(true, CliLayout.class);
        CliLayout layout;
        if (layoutEnv == layoutSysEnv)
            layout = layoutEnv;
        else
            layout = layoutEnv == defaultLayout ? layoutSysEnv : layoutEnv;
        switch (layout)
        {
            case AIRLINE:
