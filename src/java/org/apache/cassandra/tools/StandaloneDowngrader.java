...
public class StandaloneDowngrader
{
    ...
    public static void main(String args[])
    {
        ...
        // verifies version is understood
        Downgrader.forWriting(options.version);
        ...
        try
        {
            ...
            for (Map.Entry<Descriptor, Set<Component>> entry : lister.list().entrySet())
            {
                ...
                try
                {
                    ...
                    if (sstable.descriptor.version.getVersion().compareTo(options.version) <= 0)
                    {
            ...
            for (SSTableReader sstable : readers)
            {
                ...
                {
                    Downgrader downgrader = new Downgrader(options.version, cfs, txn, handler, sstable.descriptor.formatType);
    ...
    private static class Options
    {
        ...
        private Options(String keyspace, String cf, String version, String snapshot)
        {
        ...
        public static Options parseArgs(String cmdArgs[])
        {
            ...
            try
            {
                ...
                {
                    String msg = args.length < 3 ? "Missing arguments" : "Too many arguments";
                ...
                Options opts = new Options(keyspace, cf, version, snapshot);
        ...
        {
            String usage = String.format("%s [options] <keyspace> <cf> <version> [snapshot]", TOOL_NAME);
            StringBuilder header = new StringBuilder();
            header.append("--\n");
            header.append("Downgrade the sstables in the given cf (or snapshot) to the specified version of Cassandra. " );
            header.append("This operation will rewrite the sstables in the specified cf to match the " );
            header.append("specified version of Cassandra. \n");
            header.append("The snapshot option will only downgrade the specified snapshot. Downgrading " );
            header.append("snapshots is required before attempting to restore a snapshot taken in a " );
            header.append("major version newer than the major version Cassandra to run. " );
